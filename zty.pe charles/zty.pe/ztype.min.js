...
(function (window) {
    "use strict"; Number.prototype.map = function (istart, istop, ostart, ostop) { return ostart + (ostop - ostart) * ((this - istart) / (istop - istart)); }; Number.prototype.limit = function (min, max) { return Math.min(max, Math.max(min, this)); }; Number.prototype.round = function (precision) { precision = Math.pow(10, precision || 0); return Math.round(this * precision) / precision; }; Number.prototype.floor = function () { return Math.floor(this); }; Number.prototype.ceil = function () { return Math.ceil(this); }; Number.prototype.toInt = function () { return (this | 0); }; Number.prototype.toRad = function () { return (this / 180) * Math.PI; }; Number.prototype.toDeg = function () { return (this * 180) / Math.PI; }; Object.defineProperty(Array.prototype, 'erase', {
        value: function (item) {
            for (var i = this.length; i--;) { if (this[i] === item) { this.splice(i, 1); } }
            return this;
        }
    }); Object.defineProperty(Array.prototype, 'random', { value: function (item) { return this[Math.floor(Math.random() * this.length)]; } }); Function.prototype.bind = Function.prototype.bind || function (oThis) {
        if (typeof this !== "function") { throw new TypeError("Function.prototype.bind - what is trying to be bound is not callable"); }
        var aArgs = Array.prototype.slice.call(arguments, 1), fToBind = this, fNOP = function () { }, fBound = function () { return fToBind.apply((this instanceof fNOP && oThis ? this : oThis), aArgs.concat(Array.prototype.slice.call(arguments))); }; fNOP.prototype = this.prototype; fBound.prototype = new fNOP(); return fBound;
    }; window.ig = {
        game: null, debug: null, version: '1.24', global: window, modules: {}, resources: [], ready: false, baked: false, nocache: '', ua: {}, prefix: (window.ImpactPrefix || ''), lib: 'lib/', _current: null, _loadQueue: [], _waitForOnload: 0, $: function (selector) { return selector.charAt(0) == '#' ? document.getElementById(selector.substr(1)) : document.getElementsByTagName(selector); }, $new: function (name) { return document.createElement(name); }, copy: function (object) {
            if (!object || typeof (object) != 'object' || object instanceof HTMLElement || object instanceof ig.Class) { return object; }
            else if (object instanceof Array) {
                var c = []; for (var i = 0, l = object.length; i < l; i++) { c[i] = ig.copy(object[i]); }
                return c;
            }
            else {
                var c = {}; for (var i in object) { c[i] = ig.copy(object[i]); }
                return c;
            }
        }, merge: function (original, extended) {
            for (var key in extended) {
                var ext = extended[key]; if (typeof (ext) != 'object' || ext instanceof HTMLElement || ext instanceof ig.Class || ext === null) { original[key] = ext; }
                else {
            ...
            return values;
        }, setVendorAttribute: function (el, attr, val) { var uc = attr.charAt(0).toUpperCase() + attr.substr(1); el[attr] = el['ms' + uc] = el['moz' + uc] = el['webkit' + uc] = el['o' + uc] = val; }, getVendorAttribute: function (el, attr) { var uc = attr.charAt(0).toUpperCase() + attr.substr(1); return el[attr] || el['ms' + uc] || el['moz' + uc] || el['webkit' + uc] || el['o' + uc]; }, normalizeVendorAttribute: function (el, attr) { var prefixedVal = ig.getVendorAttribute(el, attr); if (!el[attr] && prefixedVal) { el[attr] = prefixedVal; } }, getImagePixels: function (image, x, y, width, height) { var canvas = ig.$new('canvas'); canvas.width = image.width; canvas.height = image.height; var ctx = canvas.getContext('2d'); ig.System.SCALE.CRISP(canvas, ctx); var ratio = ig.getVendorAttribute(ctx, 'backingStorePixelRatio') || 1; ig.normalizeVendorAttribute(ctx, 'getImageDataHD'); var realWidth = image.width / ratio, realHeight = image.height / ratio; canvas.width = Math.ceil(realWidth); canvas.height = Math.ceil(realHeight); ctx.drawImage(image, 0, 0, realWidth, realHeight); return (ratio === 1) ? ctx.getImageData(x, y, width, height) : ctx.getImageDataHD(x, y, width, height); }, module: function (name) {
            ...
            ig._current = { name: name, requires: [], loaded: false, body: null }; ig.modules[name] = ig._current; ig._loadQueue.push(ig._current); return ig;
        }, requires: function () { ig._current.requires = Array.prototype.slice.call(arguments); return ig; }, defines: function (body) { ig._current.body = body; ig._current = null; ig._initDOMReady(); }, addResource: function (resource) { ig.resources.push(resource); }, setNocache: function (set) { ig.nocache = set ? '?' + Date.now() : ''; }, log: function () { }, assert: function (condition, msg) { }, show: function (name, number) { }, mark: function (msg, color) { }, _loadScript: function (name, requiredFrom) { ig.modules[name] = { name: name, requires: [], loaded: false, body: null }; ig._waitForOnload++; var path = ig.prefix + ig.lib + name.replace(/\./g, '/') + '.js' + ig.nocache; var script = ig.$new('script'); script.type = 'text/javascript'; script.src = path; script.onload = function () { ig._waitForOnload--; ig._execModules(); }; script.onerror = function () { throw ('Failed to load module ' + name + ' at ' + path + ' ' + 'required from ' + requiredFrom); }; ig.$('head')[0].appendChild(script); }, _execModules: function () {
            var modulesLoaded = false; for (var i = 0; i < ig._loadQueue.length; i++) {
                var m = ig._loadQueue[i]; var dependenciesLoaded = true; for (var j = 0; j < m.requires.length; j++) {
                    var name = m.requires[j]; if (!ig.modules[name]) { dependenciesLoaded = false; ig._loadScript(name, m.name); }
            ...
            if (modulesLoaded) { ig._execModules(); }
                ...
                var unresolved = []; for (var i = 0; i < ig._loadQueue.length; i++) {
                    var unloaded = []; var requires = ig._loadQueue[i].requires; for (var j = 0; j < requires.length; j++) { var m = ig.modules[requires[j]]; if (!m || !m.loaded) { unloaded.push(requires[j]); } }
            ...
            else { document.addEventListener('DOMContentLoaded', ig._DOMReady, false); window.addEventListener('load', ig._DOMReady, false); }
        }
    }; ig.normalizeVendorAttribute(window, 'requestAnimationFrame'); if (window.requestAnimationFrame) {
        var next = 1, anims = {}; window.ig.setAnimation = function (callback, element) {
            var current = next++; anims[current] = true; var animate = function () {
                if (!anims[current]) { return; }
                window.requestAnimationFrame(animate, element); callback();
            }; window.requestAnimationFrame(animate, element); return current;
    ...
    var initializing = false, fnTest = /xyz/.test(function () { xyz; }) ? /\bparent\b/ : /.*/; var lastClassId = 0; window.ig.Class = function () { }; var inject = function (prop) {
        var proto = this.prototype; var parent = {}; for (var name in prop) {
            if (typeof (prop[name]) == "function" && typeof (proto[name]) == "function" && fnTest.test(prop[name])) { parent[name] = proto[name]; proto[name] = (function (name, fn) { return function () { var tmp = this.parent; this.parent = parent[name]; var ret = fn.apply(this, arguments); this.parent = tmp; return ret; }; })(name, prop[name]); }
            else { proto[name] = prop[name]; }
        }
    }; window.ig.Class.extend = function (prop) {
        var parent = this.prototype; initializing = true; var prototype = new this(); initializing = false; for (var name in prop) {
            if (typeof (prop[name]) == "function" && typeof (parent[name]) == "function" && fnTest.test(prop[name])) { prototype[name] = (function (name, fn) { return function () { var tmp = this.parent; this.parent = parent[name]; var ret = fn.apply(this, arguments); this.parent = tmp; return ret; }; })(name, prop[name]); }
        ...
        Class.prototype = prototype; Class.prototype.constructor = Class; Class.extend = window.ig.Class.extend; Class.inject = inject; Class.classId = prototype.classId = ++lastClassId; return Class;
    }; if (window.ImpactMixin) { ig.merge(ig, window.ImpactMixin); }
})(window);

// lib/impact/image.js
ig.baked = true; ig.module('impact.image').defines(function () {
    "use strict"; ig.Image = ig.Class.extend({
        data: null, width: 0, height: 0, loaded: false, failed: false, loadCallback: null, path: '', staticInstantiate: function (path) { return ig.Image.cache[path] || null; }, init: function (path) { this.path = path; this.load(); }, load: function (loadCallback) {
            if (this.loaded) {
        ...
        }, reload: function () { this.loaded = false; this.data = new Image(); this.data.onload = this.onload.bind(this); this.data.src = this.path + '?' + Date.now(); }, onload: function (event) {
            this.width = this.data.width; this.height = this.data.height; this.loaded = true; if (ig.system.scale != 1) { this.resize(ig.system.scale); }
            if (this.loadCallback) { this.loadCallback(this.path, true); }
        }, onerror: function (event) { this.failed = true; if (this.loadCallback) { this.loadCallback(this.path, false); } }, resize: function (scale) {
            ...
            scaledCtx.putImageData(scaledPixels, 0, 0); this.data = scaled;
    ...
    }); ig.Image.drawCount = 0; ig.Image.cache = {}; ig.Image.reloadCache = function () { for (var path in ig.Image.cache) { ig.Image.cache[path].reload(); } };
});

// lib/impact/font.js
ig.baked = true; ig.module('impact.font').requires('impact.image').defines(function () {
    "use strict"; ig.Font = ig.Image.extend({
        widthMap: [], indices: [], firstChar: 32, alpha: 1, letterSpacing: 1, lineSpacing: 0, onload: function (ev) { this._loadMetrics(this.data); this.parent(ev); }, widthForString: function (text) {
            if (text.indexOf('\n') !== -1) {
            ...
            var width = 0; for (var i = 0; i < text.length; i++) { width += this.widthMap[text.charCodeAt(i) - this.firstChar] + this.letterSpacing; }
            return width;
        }, heightForString: function (text) { return text.split('\n').length * (this.height + this.lineSpacing); }, draw: function (text, x, y, align) {
            ...
            var scale = ig.system.scale; var charX = this.indices[c] * scale; var charY = 0; var charWidth = this.widthMap[c] * scale; var charHeight = (this.height - 2) * scale; ig.system.context.drawImage(this.data, charX, charY, charWidth, charHeight, ig.system.getDrawPos(targetX), ig.system.getDrawPos(targetY), charWidth, charHeight); return this.widthMap[c] + this.letterSpacing;
        }, _loadMetrics: function (image) {
            this.height = image.height - 1; this.widthMap = []; this.indices = []; var px = ig.getImagePixels(image, 0, image.height - 1, image.width, 1); var currentChar = 0; var currentWidth = 0; for (var x = 0; x < image.width; x++) {
                var index = x * 4 + 3; if (px.data[index] > 127) { currentWidth++; }
...
// lib/impact/sound.js
ig.baked = true; ig.module('impact.sound').defines(function () {
    "use strict"; ig.SoundManager = ig.Class.extend({
        clips: {}, volume: 1, format: null, init: function () {
            ...
            if (ig.Sound.enabled && ig.Sound.useWebAudio) { this.audioContext = new AudioContext(); this.boundWebAudioUnlock = this.unlockWebAudio.bind(this); document.addEventListener('touchend', this.boundWebAudioUnlock, false); document.addEventListener('click', this.boundWebAudioUnlock, false); }
        }, unlockWebAudio: function () {
            if (this.audioContext.resume) { this.audioContext.resume(); }
            var buffer = this.audioContext.createBuffer(1, 1, 44100); var source = this.audioContext.createBufferSource(); source.buffer = buffer; source.connect(this.audioContext.destination); source.start(0); document.removeEventListener('touchend', this.boundWebAudioUnlock, false); document.removeEventListener('click', this.boundWebAudioUnlock, false);
        }, load: function (path, multiChannel, loadCallback) {
            if (multiChannel && ig.Sound.useWebAudio) { return this.loadWebAudio(path, multiChannel, loadCallback); }
            else { return this.loadHTML5Audio(path, multiChannel, loadCallback); }
        }, loadWebAudio: function (path, multiChannel, loadCallback) {
            var realPath = ig.prefix + path.replace(/[^\.]+$/, this.format.ext) + ig.nocache; if (this.clips[path]) { return this.clips[path]; }
            ...
            var realPath = ig.prefix + path.replace(/[^\.]+$/, this.format.ext) + ig.nocache; if (this.clips[path]) {
            ...
            var clip = new Audio(realPath); if (loadCallback) {
                if (ig.ua.mobile) { setTimeout(function () { loadCallback(path, true, null); }, 0); }
            ...
            var channels = this.clips[path]; if (channels && channels instanceof ig.Sound.WebAudioSource) { return channels; }
        ...
        tracks: [], namedTracks: {}, currentTrack: null, currentIndex: 0, random: false, _volume: 1, _loop: true, _fadeInterval: 0, _fadeTimer: null, _endedCallbackBound: null, init: function () { this._endedCallbackBound = this._endedCallback.bind(this); Object.defineProperty(this, "volume", { get: this.getVolume.bind(this), set: this.setVolume.bind(this) }); Object.defineProperty(this, "loop", { get: this.getLooping.bind(this), set: this.setLooping.bind(this) }); }, add: function (music, name) {
            ...
            this.stop(); this.currentIndex = this.random ? Math.floor(Math.random() * this.tracks.length) : (this.currentIndex + 1) % this.tracks.length; this.currentTrack = this.tracks[this.currentIndex]; this.play();
        }, pause: function () {
            ...
            this.currentTrack.volume = this.volume; this.currentTrack.play();
        }, getLooping: function () { return this._loop; }, setLooping: function (l) { this._loop = l; for (var i in this.tracks) { this.tracks[i].loop = l; } }, getVolume: function () { return this._volume; }, setVolume: function (v) { this._volume = v.limit(0, 1); for (var i in this.tracks) { this.tracks[i].volume = this._volume; } }, fadeToTrack: function (time, name) {
            ...
            this._nextTrack = this.namedTracks[name]; this._nextTrack.volume = 0; this._nextTrack.play(); clearInterval(this._fadeInterval); this.fadeTimer = new ig.Timer(time); this._fadeInterval = setInterval(this._fadeStepNext.bind(this), 50);
        }, _fadeStepNext: function () {
            var v = this.fadeTimer.delta().map(-this.fadeTimer.target, 0, 1, 0).limit(0, 1); if (v <= 0.01) {
                clearInterval(this._fadeInterval); if (this.currentTrack) { this.currentTrack.pause(); this.currentTrack.volume = this._volume; }
                this.currentTrack = this._nextTrack; this.currentTrack.volume = this._volume;
            }
            else {
                if (this.currentTrack) { this.currentTrack.volume = v * this._volume; }
                this._nextTrack.volume = (1 - v) * this._volume;
            }
        }, fadeOut: function (time) {
            if (!this.currentTrack) { return; }
            clearInterval(this._fadeInterval); this.fadeTimer = new ig.Timer(time); this._fadeInterval = setInterval(this._fadeStep.bind(this), 50);
        }, _fadeStep: function () {
            var v = this.fadeTimer.delta().map(-this.fadeTimer.target, 0, 1, 0).limit(0, 1) * this._volume; if (v <= 0.01) { this.stop(); this.currentTrack.volume = this._volume; clearInterval(this._fadeInterval); }
        ...
        path: '', volume: 1, currentClip: null, multiChannel: true, _loop: false, _fadeInterval: 0, init: function (path, multiChannel) { this.path = path; this.multiChannel = (multiChannel !== false); Object.defineProperty(this, "loop", { get: this.getLooping.bind(this), set: this.setLooping.bind(this) }); this.load(); }, fadeIn: function (time) { this.currentClip = ig.soundManager.get(this.path); this.currentClip.loop = this._loop; this.currentClip.volume = 0; this.currentClip.play(); clearInterval(this._fadeInterval); this.fadeTimer = new ig.Timer(time); this._fadeInterval = setInterval(this._fadeStep.bind(this), 100); }, _fadeStep: function () {
            var v = this.fadeTimer.delta().map(-this.fadeTimer.target, 0, 0, 1).limit(0, 1) * this.volume; if (v <= 0.01) { this.stop(); this.currentClip.volume = this.volume; clearInterval(this._fadeInterval); }
            else { this.currentClip.volume = v; }
        }, getLooping: function () { return this._loop; }, setLooping: function (loop) { this._loop = loop; if (this.currentClip) { this.currentClip.loop = loop; } }, load: function (loadCallback) {
            if (!ig.Sound.enabled) {
                if (loadCallback) { loadCallback(this.path, true); }
                return;
            }
...
ig.baked = true; ig.module('impact.loader').requires('impact.image', 'impact.font', 'impact.sound').defines(function () {
    "use strict"; ig.Loader = ig.Class.extend({
        resources: [], gameClass: null, status: 0, done: false, _unloaded: [], _drawStatus: 0, _intervalId: 0, _loadCallbackBound: null, init: function (gameClass, resources) { this.gameClass = gameClass; this.resources = resources; this._loadCallbackBound = this._loadCallback.bind(this); for (var i = 0; i < this.resources.length; i++) { this._unloaded.push(this.resources[i].path); } }, load: function () {
            ig.system.clear('#000'); if (!this.resources.length) { this.end(); return; }
            for (var i = 0; i < this.resources.length; i++) { this.loadResource(this.resources[i]); }
            this._intervalId = setInterval(this.draw.bind(this), 16);
        }, loadResource: function (res) { res.load(this._loadCallbackBound); }, end: function () {
            if (this.done) { return; }
            this.done = true; clearInterval(this._intervalId); ig.system.setGame(this.gameClass);
        }, draw: function () { this._drawStatus += (this.status - this._drawStatus) / 5; var s = ig.system.scale; var w = ig.system.width * 0.6; var h = ig.system.height * 0.1; var x = ig.system.width * 0.5 - w / 2; var y = ig.system.height * 0.5 - h / 2; ig.system.context.fillStyle = '#000'; ig.system.context.fillRect(0, 0, 480, 320); ig.system.context.fillStyle = '#fff'; ig.system.context.fillRect(x * s, y * s, w * s, h * s); ig.system.context.fillStyle = '#000'; ig.system.context.fillRect(x * s + s, y * s + s, w * s - s - s, h * s - s - s); ig.system.context.fillStyle = '#fff'; ig.system.context.fillRect(x * s, y * s, w * s * this._drawStatus, h * s); }, _loadCallback: function (path, status) {
...
ig.baked = true; ig.module('impact.system').requires('impact.timer', 'impact.image').defines(function () {
    "use strict"; ig.System = ig.Class.extend({
        fps: 30, width: 320, height: 240, realWidth: 320, realHeight: 240, scale: 1, tick: 0, animationId: 0, newGameClass: null, running: false, delegate: null, clock: null, canvas: null, context: null, init: function (canvasId, fps, width, height, scale) {
            this.fps = fps; this.clock = new ig.Timer(); this.canvas = ig.$(canvasId); this.resize(width, height, scale); this.context = this.canvas.getContext('2d'); this.getDrawPos = ig.System.drawMode; if (this.scale != 1) { ig.System.scaleMode = ig.System.SCALE.CRISP; }
            ig.System.scaleMode(this.canvas, this.context);
        }, resize: function (width, height, scale) { this.width = width; this.height = height; this.scale = scale || this.scale; this.realWidth = this.width * this.scale; this.realHeight = this.height * this.scale; this.canvas.width = this.realWidth; this.canvas.height = this.realHeight; }, setGame: function (gameClass) {
            if (this.running) { this.newGameClass = gameClass; }
            else { this.setGameNow(gameClass); }
        }, setGameNow: function (gameClass) { ig.game = new (gameClass)(); ig.system.setDelegate(ig.game); }, setDelegate: function (object) { if (typeof (object.run) == 'function') { this.delegate = object; this.startRunLoop(); } else { throw ('System.setDelegate: No run() function in object'); } }, stopRunLoop: function () { ig.clearAnimation(this.animationId); this.running = false; }, startRunLoop: function () { this.stopRunLoop(); this.animationId = ig.setAnimation(this.run.bind(this), this.canvas); this.running = true; }, clear: function (color) { this.context.fillStyle = color; this.context.fillRect(0, 0, this.realWidth, this.realHeight); }, run: function () { ig.Timer.step(); this.tick = this.clock.tick(); this.delegate.run(); ig.input.clearPressed(); if (this.newGameClass) { this.setGameNow(this.newGameClass); this.newGameClass = null; } }, getDrawPos: null
    }); ig.System.DRAW = { AUTHENTIC: function (p) { return Math.round(p) * this.scale; }, SMOOTH: function (p) { return Math.round(p * this.scale); }, SUBPIXEL: function (p) { return p * this.scale; } }; ig.System.drawMode = ig.System.DRAW.SMOOTH; ig.System.SCALE = { CRISP: function (canvas, context) { ig.setVendorAttribute(context, 'imageSmoothingEnabled', false); canvas.style.imageRendering = '-moz-crisp-edges'; canvas.style.imageRendering = '-o-crisp-edges'; canvas.style.imageRendering = '-webkit-optimize-contrast'; canvas.style.imageRendering = 'crisp-edges'; canvas.style.msInterpolationMode = 'nearest-neighbor'; }, SMOOTH: function (canvas, context) { ig.setVendorAttribute(context, 'imageSmoothingEnabled', true); canvas.style.imageRendering = ''; canvas.style.msInterpolationMode = ''; } }; ig.System.scaleMode = ig.System.SCALE.SMOOTH;
});

// lib/impact/input.js
ig.baked = true; ig.module('impact.input').defines(function () {
    "use strict"; ig.KEY = { 'MOUSE1': -1, 'MOUSE2': -3, 'MWHEEL_UP': -4, 'MWHEEL_DOWN': -5, 'BACKSPACE': 8, 'TAB': 9, 'ENTER': 13, 'PAUSE': 19, 'CAPS': 20, 'ESC': 27, 'SPACE': 32, 'PAGE_UP': 33, 'PAGE_DOWN': 34, 'END': 35, 'HOME': 36, 'LEFT_ARROW': 37, 'UP_ARROW': 38, 'RIGHT_ARROW': 39, 'DOWN_ARROW': 40, 'INSERT': 45, 'DELETE': 46, '_0': 48, '_1': 49, '_2': 50, '_3': 51, '_4': 52, '_5': 53, '_6': 54, '_7': 55, '_8': 56, '_9': 57, 'A': 65, 'B': 66, 'C': 67, 'D': 68, 'E': 69, 'F': 70, 'G': 71, 'H': 72, 'I': 73, 'J': 74, 'K': 75, 'L': 76, 'M': 77, 'N': 78, 'O': 79, 'P': 80, 'Q': 81, 'R': 82, 'S': 83, 'T': 84, 'U': 85, 'V': 86, 'W': 87, 'X': 88, 'Y': 89, 'Z': 90, 'NUMPAD_0': 96, 'NUMPAD_1': 97, 'NUMPAD_2': 98, 'NUMPAD_3': 99, 'NUMPAD_4': 100, 'NUMPAD_5': 101, 'NUMPAD_6': 102, 'NUMPAD_7': 103, 'NUMPAD_8': 104, 'NUMPAD_9': 105, 'MULTIPLY': 106, 'ADD': 107, 'SUBSTRACT': 109, 'DECIMAL': 110, 'DIVIDE': 111, 'F1': 112, 'F2': 113, 'F3': 114, 'F4': 115, 'F5': 116, 'F6': 117, 'F7': 118, 'F8': 119, 'F9': 120, 'F10': 121, 'F11': 122, 'F12': 123, 'SHIFT': 16, 'CTRL': 17, 'ALT': 18, 'PLUS': 187, 'COMMA': 188, 'MINUS': 189, 'PERIOD': 190 }; ig.Input = ig.Class.extend({
        bindings: {}, actions: {}, presses: {}, locks: {}, delayedKeyup: {}, isUsingMouse: false, isUsingKeyboard: false, isUsingAccelerometer: false, mouse: { x: 0, y: 0 }, accel: { x: 0, y: 0, z: 0 }, initMouse: function () {
            if (this.isUsingMouse) { return; }
            this.isUsingMouse = true; var mouseWheelBound = this.mousewheel.bind(this); ig.system.canvas.addEventListener('mousewheel', mouseWheelBound, false); ig.system.canvas.addEventListener('DOMMouseScroll', mouseWheelBound, false); ig.system.canvas.addEventListener('contextmenu', this.contextmenu.bind(this), false); ig.system.canvas.addEventListener('mousedown', this.keydown.bind(this), false); ig.system.canvas.addEventListener('mouseup', this.keyup.bind(this), false); ig.system.canvas.addEventListener('mousemove', this.mousemove.bind(this), false); if (ig.ua.touchDevice) { ig.system.canvas.addEventListener('touchstart', this.keydown.bind(this), false); ig.system.canvas.addEventListener('touchend', this.keyup.bind(this), false); ig.system.canvas.addEventListener('touchmove', this.mousemove.bind(this), false); ig.system.canvas.addEventListener('MSPointerDown', this.keydown.bind(this), false); ig.system.canvas.addEventListener('MSPointerUp', this.keyup.bind(this), false); ig.system.canvas.addEventListener('MSPointerMove', this.mousemove.bind(this), false); ig.system.canvas.style.msTouchAction = 'none'; }
        }, initKeyboard: function () {
            if (this.isUsingKeyboard) { return; }
            this.isUsingKeyboard = true; window.addEventListener('keydown', this.keydown.bind(this), false); window.addEventListener('keyup', this.keyup.bind(this), false);
        }, initAccelerometer: function () {
            if (this.isUsingAccelerometer) { return; }
            this.isUsingAccelerometer = true; window.addEventListener('devicemotion', this.devicemotion.bind(this), false);
        }, mousewheel: function (event) { var delta = event.wheelDelta ? event.wheelDelta : (event.detail * -1); var code = delta > 0 ? ig.KEY.MWHEEL_UP : ig.KEY.MWHEEL_DOWN; var action = this.bindings[code]; if (action) { this.actions[action] = true; this.presses[action] = true; this.delayedKeyup[action] = true; event.preventDefault(); } }, mousemove: function (event) {
            ...
            var ev = event.touches ? event.touches[0] : event; this.mouse.x = (ev.clientX - pos.left) / scale; this.mouse.y = (ev.clientY - pos.top) / scale;
        }, contextmenu: function (event) { if (this.bindings[ig.KEY.MOUSE2]) { event.preventDefault(); } }, keydown: function (event) {
            ...
            var action = this.bindings[code]; if (action) {
                this.actions[action] = true; if (!this.locks[action]) { this.presses[action] = true; this.locks[action] = true; }
                event.preventDefault();
            }
        }, keyup: function (event) {
            var tag = event.target.tagName; if (tag == 'INPUT' || tag == 'TEXTAREA') { return; }
            var code = event.type == 'keyup' ? event.keyCode : (event.button == 2 ? ig.KEY.MOUSE2 : ig.KEY.MOUSE1); var action = this.bindings[code]; if (action) { this.delayedKeyup[action] = true; event.preventDefault(); }
        }, devicemotion: function (event) { this.accel = event.accelerationIncludingGravity; }, bind: function (key, action) {
            if (key < 0) { this.initMouse(); }
            else if (key > 0) { this.initKeyboard(); }
            this.bindings[key] = action;
        }, bindTouch: function (selector, action) { var element = ig.$(selector); var that = this; element.addEventListener('touchstart', function (ev) { that.touchStart(ev, action); }, false); element.addEventListener('touchend', function (ev) { that.touchEnd(ev, action); }, false); element.addEventListener('MSPointerDown', function (ev) { that.touchStart(ev, action); }, false); element.addEventListener('MSPointerUp', function (ev) { that.touchEnd(ev, action); }, false); }, unbind: function (key) { var action = this.bindings[key]; this.delayedKeyup[action] = true; this.bindings[key] = null; }, unbindAll: function () { this.bindings = {}; this.actions = {}; this.presses = {}; this.locks = {}; this.delayedKeyup = {}; }, state: function (action) { return this.actions[action]; }, pressed: function (action) { return this.presses[action]; }, released: function (action) { return !!this.delayedKeyup[action]; }, clearPressed: function () {
            for (var action in this.delayedKeyup) { this.actions[action] = false; this.locks[action] = false; }
            this.delayedKeyup = {}; this.presses = {};
        }, touchStart: function (event, action) { this.actions[action] = true; this.presses[action] = true; event.preventDefault(); return false; }, touchEnd: function (event, action) { this.delayedKeyup[action] = true; event.preventDefault(); return false; }
    });
});
...
ig.baked = true; ig.module('impact.animation').requires('impact.timer', 'impact.image').defines(function () {
    "use strict"; ig.AnimationSheet = ig.Class.extend({ width: 8, height: 8, image: null, init: function (path, width, height) { this.width = width; this.height = height; this.image = new ig.Image(path); } }); ig.Animation = ig.Class.extend({
        sheet: null, timer: null, sequence: [], flip: { x: false, y: false }, pivot: { x: 0, y: 0 }, frame: 0, tile: 0, loopCount: 0, alpha: 1, angle: 0, init: function (sheet, frameTime, sequence, stop) { this.sheet = sheet; this.pivot = { x: sheet.width / 2, y: sheet.height / 2 }; this.timer = new ig.Timer(); this.frameTime = frameTime; this.sequence = sequence; this.stop = !!stop; this.tile = this.sequence[0]; }, rewind: function () { this.timer.set(); this.loopCount = 0; this.frame = 0; this.tile = this.sequence[0]; return this; }, gotoFrame: function (f) { this.timer.set(this.frameTime * -f - 0.0001); this.update(); }, gotoRandomFrame: function () { this.gotoFrame(Math.floor(Math.random() * this.sequence.length)) }, update: function () {
            var frameTotal = Math.floor(this.timer.delta() / this.frameTime); this.loopCount = Math.floor(frameTotal / this.sequence.length); if (this.stop && this.loopCount > 0) { this.frame = this.sequence.length - 1; }
...
ig.baked = true; ig.module('impact.entity').requires('impact.animation', 'impact.impact').defines(function () {
    "use strict"; ig.Entity = ig.Class.extend({
        id: 0, settings: {}, size: { x: 16, y: 16 }, offset: { x: 0, y: 0 }, pos: { x: 0, y: 0 }, last: { x: 0, y: 0 }, vel: { x: 0, y: 0 }, accel: { x: 0, y: 0 }, friction: { x: 0, y: 0 }, maxVel: { x: 100, y: 100 }, zIndex: 0, gravityFactor: 1, standing: false, bounciness: 0, minBounceVelocity: 40, anims: {}, animSheet: null, currentAnim: null, health: 10, type: 0, checkAgainst: 0, collides: 0, _killed: false, slopeStanding: { min: (44).toRad(), max: (136).toRad() }, init: function (x, y, settings) { this.id = ++ig.Entity._lastId; this.pos.x = this.last.x = x; this.pos.y = this.last.y = y; ig.merge(this, settings); }, reset: function (x, y, settings) { var proto = this.constructor.prototype; this.pos.x = x; this.pos.y = y; this.last.x = x; this.last.y = y; this.vel.x = proto.vel.x; this.vel.y = proto.vel.y; this.accel.x = proto.accel.x; this.accel.y = proto.accel.y; this.health = proto.health; this._killed = proto._killed; this.standing = proto.standing; this.type = proto.type; this.checkAgainst = proto.checkAgainst; this.collides = proto.collides; ig.merge(this, settings); }, addAnim: function (name, frameTime, sequence, stop) {
            ...
            if (res.collision.slope) {
                var s = res.collision.slope; if (this.bounciness > 0) { var proj = this.vel.x * s.nx + this.vel.y * s.ny; this.vel.x = (this.vel.x - s.nx * proj * 2) * this.bounciness; this.vel.y = (this.vel.y - s.ny * proj * 2) * this.bounciness; }
        ...
        var weak = null; if (a.collides == ig.Entity.COLLIDES.LITE || b.collides == ig.Entity.COLLIDES.FIXED) { weak = a; }
        ...
        if (a.last.x + a.size.x > b.last.x && a.last.x < b.last.x + b.size.x) {
            if (a.last.y < b.last.y) { ig.Entity.seperateOnYAxis(a, b, weak); }
            ...
            if (a.last.x < b.last.x) { ig.Entity.seperateOnXAxis(a, b, weak); }
        ...
        var nudge = (left.pos.x + left.size.x - right.pos.x); if (weak) { var strong = left === weak ? right : left; weak.vel.x = -weak.vel.x * weak.bounciness + strong.vel.x; var resWeak = ig.game.collisionMap.trace(weak.pos.x, weak.pos.y, weak == left ? -nudge : nudge, 0, weak.size.x, weak.size.y); weak.pos.x = resWeak.pos.x; }
        ...
        var nudge = (top.pos.y + top.size.y - bottom.pos.y); if (weak) {
            var strong = top === weak ? bottom : top; weak.vel.y = -weak.vel.y * weak.bounciness + strong.vel.y; var nudgeX = 0; if (weak == top && Math.abs(weak.vel.y - strong.vel.y) < weak.minBounceVelocity) { weak.standing = true; nudgeX = strong.vel.x * ig.system.tick; }
            ...
            var resTop = ig.game.collisionMap.trace(top.pos.x, top.pos.y, 0, -(top.pos.y + top.size.y - bottom.pos.y), top.size.x, top.size.y); top.pos.y = resTop.pos.y; if (top.bounciness > 0 && top.vel.y > top.minBounceVelocity) { top.vel.y *= -top.bounciness; }
...
// lib/impact/map.js
ig.baked = true; ig.module('impact.map').defines(function () {
    "use strict"; ig.Map = ig.Class.extend({
        tilesize: 8, width: 1, height: 1, data: [[]], name: null, init: function (tilesize, data) { this.tilesize = tilesize; this.data = data; this.height = data.length; this.width = data[0].length; this.pxWidth = this.width * this.tilesize; this.pxHeight = this.height * this.tilesize; }, getTile: function (x, y) {
            var tx = Math.floor(x / this.tilesize); var ty = Math.floor(y / this.tilesize); if ((tx >= 0 && tx < this.width) && (ty >= 0 && ty < this.height)) { return this.data[ty][tx]; }
            else { return 0; }
...
ig.baked = true; ig.module('impact.collision-map').requires('impact.map').defines(function () {
    "use strict"; ig.CollisionMap = ig.Map.extend({
        lastSlope: 1, tiledef: null, init: function (tilesize, data, tiledef) { this.parent(tilesize, data); this.tiledef = tiledef || ig.CollisionMap.defaultTileDef; for (var t in this.tiledef) { if (t | 0 > this.lastSlope) { this.lastSlope = t | 0; } } }, trace: function (x, y, vx, vy, objectWidth, objectHeight) {
            var res = { collision: { x: false, y: false, slope: false }, pos: { x: x, y: y }, tile: { x: 0, y: 0 } }; var steps = Math.ceil((Math.max(Math.abs(vx), Math.abs(vy)) + 0.1) / this.tilesize); if (steps > 1) {
                var sx = vx / steps; var sy = vy / steps; for (var i = 0; i < steps && (sx || sy); i++) {
                    this._traceStep(res, x, y, sx, sy, objectWidth, objectHeight, vx, vy, i); x = res.pos.x; y = res.pos.y; if (res.collision.x) { sx = 0; vx = 0; }
            ...
            res.pos.x += vx; res.pos.y += vy; var t = 0; if (vx) {
                var pxOffsetX = (vx > 0 ? width : 0); var tileOffsetX = (vx < 0 ? this.tilesize : 0); var firstTileY = Math.max(Math.floor(y / this.tilesize), 0); var lastTileY = Math.min(Math.ceil((y + height) / this.tilesize), this.height); var tileX = Math.floor((res.pos.x + pxOffsetX) / this.tilesize); var prevTileX = Math.floor((x + pxOffsetX) / this.tilesize); if (step > 0 || tileX == prevTileX || prevTileX < 0 || prevTileX >= this.width) { prevTileX = -1; }
            ...
            if (vy) {
                var pxOffsetY = (vy > 0 ? height : 0); var tileOffsetY = (vy < 0 ? this.tilesize : 0); var firstTileX = Math.max(Math.floor(res.pos.x / this.tilesize), 0); var lastTileX = Math.min(Math.ceil((res.pos.x + width) / this.tilesize), this.width); var tileY = Math.floor((res.pos.y + pxOffsetY) / this.tilesize); var prevTileY = Math.floor((y + pxOffsetY) / this.tilesize); if (step > 0 || tileY == prevTileY || prevTileY < 0 || prevTileY >= this.height) { prevTileY = -1; }
            ...
            var def = this.tiledef[t]; if (!def) { return false; }
            var lx = (tileX + def[0]) * this.tilesize, ly = (tileY + def[1]) * this.tilesize, lvx = (def[2] - def[0]) * this.tilesize, lvy = (def[3] - def[1]) * this.tilesize, solid = def[4]; var tx = x + vx + (lvy < 0 ? width : 0) - lx, ty = y + vy + (lvx > 0 ? height : 0) - ly; if (lvx * ty - lvy * tx > 0) {
...
ig.baked = true; ig.module('impact.background-map').requires('impact.map', 'impact.image').defines(function () {
    "use strict"; ig.BackgroundMap = ig.Map.extend({
        tiles: null, scroll: { x: 0, y: 0 }, distance: 1, repeat: false, tilesetName: '', foreground: false, enabled: true, preRender: false, preRenderedChunks: null, chunkSize: 512, debugChunks: false, anims: {}, init: function (tilesize, data, tileset) { this.parent(tilesize, data); this.setTileset(tileset); }, setTileset: function (tileset) { this.tilesetName = tileset instanceof ig.Image ? tileset.path : tileset; this.tiles = new ig.Image(this.tilesetName); this.preRenderedChunks = null; }, setScreenPos: function (x, y) { this.scroll.x = x / this.distance; this.scroll.y = y / this.distance; }, preRenderMapToChunks: function () { var totalWidth = this.width * this.tilesize * ig.system.scale, totalHeight = this.height * this.tilesize * ig.system.scale; this.chunkSize = Math.min(Math.max(totalWidth, totalHeight), this.chunkSize); var chunkCols = Math.ceil(totalWidth / this.chunkSize), chunkRows = Math.ceil(totalHeight / this.chunkSize); this.preRenderedChunks = []; for (var y = 0; y < chunkRows; y++) { this.preRenderedChunks[y] = []; for (var x = 0; x < chunkCols; x++) { var chunkWidth = (x == chunkCols - 1) ? totalWidth - x * this.chunkSize : this.chunkSize; var chunkHeight = (y == chunkRows - 1) ? totalHeight - y * this.chunkSize : this.chunkSize; this.preRenderedChunks[y][x] = this.preRenderChunk(x, y, chunkWidth, chunkHeight); } } }, preRenderChunk: function (cx, cy, w, h) {
            ...
            var nudgeY = 0; for (var cy = minChunkY; cy < maxChunkY; cy++) {
                var nudgeX = 0; for (var cx = minChunkX; cx < maxChunkX; cx++) {
                    var chunk = this.preRenderedChunks[cy % maxRealChunkY][cx % maxRealChunkX]; var x = -dx + cx * this.chunkSize - nudgeX; var y = -dy + cy * this.chunkSize - nudgeY; ig.system.context.drawImage(chunk, x, y); ig.Image.drawCount++; if (this.debugChunks) { ig.system.context.strokeStyle = '#f0f'; ig.system.context.strokeRect(x, y, this.chunkSize, this.chunkSize); }
                    if (this.repeat && chunk.width < this.chunkSize && x + chunk.width < ig.system.realWidth) { nudgeX += this.chunkSize - chunk.width; maxChunkX++; }
                }
                if (this.repeat && chunk.height < this.chunkSize && y + chunk.height < ig.system.realHeight) { nudgeY += this.chunkSize - chunk.height; maxChunkY++; }
            }
        }, drawTiled: function () {
            var tile = 0, anim = null, tileOffsetX = (this.scroll.x / this.tilesize).toInt(), tileOffsetY = (this.scroll.y / this.tilesize).toInt(), pxOffsetX = this.scroll.x % this.tilesize, pxOffsetY = this.scroll.y % this.tilesize, pxMinX = -pxOffsetX - this.tilesize, pxMinY = -pxOffsetY - this.tilesize, pxMaxX = ig.system.width + this.tilesize - pxOffsetX, pxMaxY = ig.system.height + this.tilesize - pxOffsetY; for (var mapY = -1, pxY = pxMinY; pxY < pxMaxY; mapY++, pxY += this.tilesize) {
                var tileY = mapY + tileOffsetY; if (tileY >= this.height || tileY < 0) {
                    if (!this.repeat) { continue; }
                    tileY = (tileY % this.height + this.height) % this.height;
                }
                for (var mapX = -1, pxX = pxMinX; pxX < pxMaxX; mapX++, pxX += this.tilesize) {
                    var tileX = mapX + tileOffsetX; if (tileX >= this.width || tileX < 0) {
                        if (!this.repeat) { continue; }
...
ig.baked = true; ig.module('impact.game').requires('impact.impact', 'impact.entity', 'impact.collision-map', 'impact.background-map').defines(function () {
    "use strict"; ig.Game = ig.Class.extend({
        clearColor: '#000000', gravity: 0, screen: { x: 0, y: 0 }, _rscreen: { x: 0, y: 0 }, entities: [], namedEntities: {}, collisionMap: ig.CollisionMap.staticNoCollision, backgroundMaps: [], backgroundAnims: {}, autoSort: false, sortBy: null, cellSize: 64, _deferredKill: [], _levelToLoad: null, _doSortEntities: false, staticInstantiate: function () { this.sortBy = this.sortBy || ig.Game.SORT.Z_INDEX; ig.game = this; return null; }, loadLevel: function (data) {
            ...
            this.sortEntities(); this.collisionMap = ig.CollisionMap.staticNoCollision; this.backgroundMaps = []; for (var i = 0; i < data.layer.length; i++) {
                var ld = data.layer[i]; if (ld.name == 'collision') { this.collisionMap = new ig.CollisionMap(ld.tilesize, ld.data); }
        ...
        }, loadLevelDeferred: function (data) { this._levelToLoad = data; }, getMapByName: function (name) {
            ...
            return null;
        }, getEntityByName: function (name) { return this.namedEntities[name]; }, getEntitiesByType: function (type) {
            var entityClass = typeof (type) === 'string' ? ig.global[type] : type; var a = []; for (var i = 0; i < this.entities.length; i++) { var ent = this.entities[i]; if (ent instanceof entityClass && !ent._killed) { a.push(ent); } }
            return a;
        }, spawnEntity: function (type, x, y, settings) {
            var entityClass = typeof (type) === 'string' ? ig.global[type] : type; if (!entityClass) { throw ("Can't spawn entity of type " + type); }
            var ent = new (entityClass)(x, y, settings || {}); this.entities.push(ent); if (ent.name) { this.namedEntities[ent.name] = ent; }
            return ent;
        }, sortEntities: function () { this.entities.sort(this.sortBy); }, sortEntitiesDeferred: function () { this._doSortEntities = true; }, removeEntity: function (ent) {
            if (ent.name) { delete this.namedEntities[ent.name]; }
            ent._killed = true; ent.type = ig.Entity.TYPE.NONE; ent.checkAgainst = ig.Entity.TYPE.NONE; ent.collides = ig.Entity.COLLIDES.NEVER; this._deferredKill.push(ent);
        ...
        }, updateEntities: function () { for (var i = 0; i < this.entities.length; i++) { var ent = this.entities[i]; if (!ent._killed) { ent.update(); } } }, draw: function () {
            if (this.clearColor) { ig.system.clear(this.clearColor); }
            this._rscreen.x = ig.system.getDrawPos(this.screen.x) / ig.system.scale; this._rscreen.y = ig.system.getDrawPos(this.screen.y) / ig.system.scale; var mapIndex; for (mapIndex = 0; mapIndex < this.backgroundMaps.length; mapIndex++) {
                var map = this.backgroundMaps[mapIndex]; if (map.foreground) { break; }
                map.setScreenPos(this.screen.x, this.screen.y); map.draw();
            }
            this.drawEntities(); for (mapIndex; mapIndex < this.backgroundMaps.length; mapIndex++) { var map = this.backgroundMaps[mapIndex]; map.setScreenPos(this.screen.x, this.screen.y); map.draw(); }
        }, drawEntities: function () { for (var i = 0; i < this.entities.length; i++) { this.entities[i].draw(); } }, checkEntities: function () {
            var hash = {}; for (var e = 0; e < this.entities.length; e++) {
                var entity = this.entities[e]; if (entity.type == ig.Entity.TYPE.NONE && entity.checkAgainst == ig.Entity.TYPE.NONE && entity.collides == ig.Entity.COLLIDES.NEVER) { continue; }
                var checked = {}, xmin = Math.floor(entity.pos.x / this.cellSize), ymin = Math.floor(entity.pos.y / this.cellSize), xmax = Math.floor((entity.pos.x + entity.size.x) / this.cellSize) + 1, ymax = Math.floor((entity.pos.y + entity.size.y) / this.cellSize) + 1; for (var x = xmin; x < xmax; x++) {
                    for (var y = ymin; y < ymax; y++) {
                        if (!hash[x]) { hash[x] = {}; hash[x][y] = [entity]; }
                        else if (!hash[x][y]) { hash[x][y] = [entity]; }
                        else {
                            var cell = hash[x][y]; for (var c = 0; c < cell.length; c++) { if (entity.touches(cell[c]) && !checked[cell[c].id]) { checked[cell[c].id] = true; ig.Entity.checkPair(entity, cell[c]); } }
                            cell.push(entity);
                        }
                    }
                }
...
ig.baked = true; ig.module('game.menus.base').requires('impact.font').defines(function () {
    MenuItem = ig.Class.extend({ getText: function () { return 'none' }, left: function () { }, right: function () { }, ok: function () { }, click: function () { this.ok(); ig.system.canvas.style.cursor = 'auto'; } }); MenuItemBack = MenuItem.extend({ getText: function () { return 'back to title'; }, ok: function () { ig.game.setTitle(); } }); Menu = ig.Class.extend({
        clearColor: null, name: null, font: new ig.Font('media/fonts/avenir-36-white.png'), fontSelected: new ig.Font('media/fonts/avenir-36-orange.png'), current: 0, itemClasses: [], items: [], scale: 1, alpha: 1, init: function () { this.width = ig.system.width; this.y = ig.system.height / 4 + 160; this.font.letterSpacing = -2; this.fontSelected.letterSpacing = -2; for (var i = 0; i < this.itemClasses.length; i++) { this.items.push(new this.itemClasses[i]()); } }, update: function () {
            ...
            var ys = this.y; var xs = ig.system.width / 2; var hoverItem = null; for (var i = 0; i < this.items.length; i++) {
                var item = this.items[i]; var w = (this.font.widthForString(item.getText()) / 2) * this.scale; if (item.y) { ys = item.y; }
            ...
            ig.system.context.save(); if (this.clearColor) { ig.system.context.fillStyle = this.clearColor; ig.system.context.fillRect(0, 0, ig.system.width, ig.system.height); }
            ...
            for (var i = 0; i < this.items.length; i++) {
                var item = this.items[i]; var t = item.getText(); if (item.y) { ys = item.y; }
...
ig.baked = true; ig.module('game.menus.about').requires('game.menus.base').defines(function () {
    MenuItemRestoreIAP = MenuItem.extend({ getText: function () { return 'Restore In-App Purchases'; }, ok: function () { ig.game.restoreIAP(); } }); MenuAbout = Menu.extend({
        background: new ig.Image('media/background/stars.jpg'), credits: new ig.Image('media/title/credits.png'), scale: 0.75, scroll: 0, itemClasses: [MenuItemRestoreIAP, MenuItemBack], init: function () {
            if (!window.Ejecta) { this.itemClasses.shift(); }
            this.parent(); this.current = 1; this.width = ig.system.width / this.scale; this.items[0].y = (ig.system.height - 130) / this.scale; if (this.items.length > 1) { this.items[1].y = (ig.system.height - 80) / this.scale; }
        }, draw: function () { var ctx = ig.system.context; ctx.save(); ctx.scale(this.scale, this.scale); this.scroll += ig.system.tick * 10; this.scroll = this.scroll % this.background.height; this.background.draw(0, this.scroll - this.background.height); this.background.draw(0, this.scroll); ctx.restore(); this.credits.draw(32, 32); this.parent(); }
    });
});

// lib/game/menus/interstitial.js
ig.baked = true; ig.module('game.menus.interstitial').requires('game.menus.base').defines(function () {
    MenuItemSkip = MenuItem.extend({ getText: function () { return 'skip'; }, ok: function () { ig.game.setTitle(); } }); MenuInterstitial = Menu.extend({
        itemClasses: [MenuItemSkip], scale: 0.75, banner: new ig.Image('media/nfn-phoboslab-works.png'), init: function () { this.parent(); this.y = (ig.system.height - 130) / this.scale; this.width = ig.system.width / this.scale; this.timer = new ig.Timer(); if (!MenuInterstitial.onclickInstalled) { MenuInterstitial.onclickInstalled = true; ig.system.canvas.addEventListener('click', function (event) { if (ig.game && ig.game.menu && ig.game.menu.onclick) { ig.game.menu.onclick(event); } }); } }, update: function () { this.parent(); var bx = ((ig.system.width - this.banner.width) / 2), by = 64, bw = this.banner.width, bh = this.banner.height; if (ig.input.mouse.x > bx && ig.input.mouse.x < bx + bw && ig.input.mouse.y > by && ig.input.mouse.y < by + bh) { ig.system.canvas.style.cursor = 'pointer'; } }, onclick: function (event) {
            var internalWidth = parseInt(ig.system.canvas.offsetWidth) || ig.system.realWidth; var scale = ig.system.scale * (internalWidth / ig.system.realWidth); var pos = { left: 0, top: 0 }; if (ig.system.canvas.getBoundingClientRect) { pos = ig.system.canvas.getBoundingClientRect(); }
            var ev = event.touches ? event.touches[0] : event; var x = (ev.clientX - pos.left) / scale; var y = (ev.clientY - pos.top) / scale; var bx = ((ig.system.width - this.banner.width) / 2), by = 64, bw = this.banner.width, bh = this.banner.height; if (x > bx && x < bx + bw && y > by && y < by + bh) { window.open('http://nofatenetmusic.bandcamp.com/album/the-phoboslab-works'); }
...
// lib/game/xhr.js
ig.baked = true; ig.module('game.xhr').defines(function () {
    ig.xhr = function (url, data, callback) {
        var post = []; if (data) { for (var key in data) { post.push(key + '=' + encodeURIComponent(data[key])); } }
...
ig.baked = true; ig.module('game.menus.stats').requires('game.menus.base', 'game.xhr').defines(function () {
    StatsView = ig.Class.extend({
        data: null, workingIndicator: ['', '.', '..', '...'], font: new ig.Font('media/fonts/avenir-36-white.png'), timer: null, width: 432, height: 216, isExcerpt: false, totalGames: 0, init: function (width, height) { this.width = width; this.height = height; this.timer = new ig.Timer(); }, submit: function (data) {
            if (window.Cocoon || window.Ejecta) {
                var games = JSON.parse(ig.storageGet('stats') || '[]'); if (games.length > 30) { games.shift(); }
                data.created = (Date.now() / 1000) | 0; games.push(data); ig.storageSet('stats', JSON.stringify(games)); this.receivedData({ games: games });
            }
            else { ig.xhr('/api/', data, this.receivedData.bind(this)); }
        }, load: function () {
            if (window.Cocoon || window.Ejecta) { }
            else { ig.xhr('/api/', { load: true }, this.receivedData.bind(this)); }
            ...
            this.isExcerpt = response.isExcerpt; this.totalGames = response.totalGames; this.scoreScale = this.roundToMagnitude(this.maxScore); this.timer.reset();
        }, roundToMagnitude: function (n) { var mag = this.magnitude(n); return Math.ceil((n * 10) / mag) * mag / 10; }, magnitude: function (n) { return Math.pow(10, Math.floor(Math.log(n) / Math.LN10)); }, draw: function (x, y) {
            if (!this.data) { this.drawLoading(x, y); }
            else { this.drawGraph(x, y); }
        }, drawLoading: function (x, y) { var scale = 0.5; var ctx = ig.system.context; ctx.save(); ctx.scale(scale, scale); ctx.globalAlpha = 0.5; var indicator = this.workingIndicator[((this.timer.delta() * 4) | 0) % this.workingIndicator.length]; this.font.draw('LOADING STATS  ' + indicator, x / scale, y / scale); ctx.restore(); }, drawGraph: function (x, y) {
            ...
            ctx.strokeStyle = 'rgba(255,255,255,0.2)'; ctx.lineWidth = 2; var lastp = null; var max = Math.min((this.data.length * this.timer.delta()), this.data.length); for (var i = 0; i < max; i++) {
                var d = this.data[i]; var v = 1 / (this.data.length - 1) * i || 0; var rx = x + (rw - 3) * v, ry = y + this.height - (this.height - 3) * (d.score / this.scoreScale); ctx.fillRect(rx, ry, 3, 3); if (lastp) {
                    ctx.beginPath(); ctx.moveTo(lastp.x + 1.5, lastp.y + 1.5); ctx.lineTo(rx + 1.5, ry + 1.5)
                    ctx.stroke();
                }
                lastp = { x: rx, y: ry };
            }
...
ig.baked = true; ig.module('game.menus.game-over').requires('game.menus.base', 'game.menus.interstitial', 'game.menus.stats').defines(function () {
    MenuItemInterstitial = MenuItem.extend({ getText: function () { return 'back to title'; }, ok: function () { ig.game.menu = new MenuInterstitial(); } }); MenuItemContinue = MenuItem.extend({ getText: function () { return 'continue'; }, ok: function () { ig.game.continue(); } }); MenuItemFeedback = MenuItem.extend({ getText: function () { return 'feedback?'; }, ok: function () { ig.game.openFeedbackForm(); } }); MenuGameOver = Menu.extend({
        itemClasses: [MenuItemBack], scale: 0.75, personalBestBadge: new ig.Image('media/ui/personal-best-badge.png'), fontTitle: new ig.Font('media/fonts/avenir-36-blue.png'), separatorBar: new ig.Image('media/ui/bar-blue.png'), init: function () {
            if (ig.doc) { this.itemClasses.unshift(MenuItemContinue); }
            this.parent(); this.y = (ig.system.height - 130) / this.scale; this.width = ig.system.width / this.scale; this.stats = new StatsView(432, ig.system.height - 500); this.stats.submit({ score: ig.game.score, wave: ig.game.wave.wave, streak: ig.game.longestStreak, accuracy: ig.game.hits ? ig.game.hits / (ig.game.hits + ig.game.misses) * 100 : 0 }); this.timer = new ig.Timer();
        }, update: function () { if (this.timer.delta() > 1.5) { this.parent(); } }, draw: function () {
            this.parent(); var xs = ig.system.width / 2; var ys = 25; var acc = ig.game.hits ? ig.game.hits / (ig.game.hits + ig.game.misses) * 100 : 0; var ctx = ig.system.context; if (ig.game.isPersonalBest) { ctx.save(); ctx.scale(0.5, 0.5); this.personalBestBadge.draw(24 / 0.5, 275 / 0.5); this.font.draw('NEW PERSONAL BEST', 60 / 0.5, 280 / 0.5, ig.Font.ALIGN.LEFT); ctx.restore(); }
...
ig.baked = true; ig.module('game.menus.settings', 'game.keyboard').requires('game.menus.base').defines(function () {
    MenuItemSoundVolume = MenuItem.extend({
        wide: true, getText: function () { return (ig.soundManager.volume * 100).round() + '%'; }, left: function () { ig.soundManager.volume = (ig.soundManager.volume - 0.1).limit(0, 1); }, right: function () { ig.soundManager.volume = (ig.soundManager.volume + 0.1).limit(0, 1); }, click: function () {
            if (ig.input.mouse.x > 240) { this.right(); } else { this.left(); }
            ig.storageSet('soundVolume', ig.soundManager.volume);
        }
    }); MenuItemMusicVolume = MenuItem.extend({
        wide: true, getText: function () { return (ig.music.volume * 100).round() + '%'; }, left: function () { ig.music.volume = (ig.music.volume - 0.1).limit(0, 1); }, right: function () { ig.music.volume = (ig.music.volume + 0.1).limit(0, 1); }, click: function () {
            if (ig.input.mouse.x > 240) { this.right(); } else { this.left(); }
            ig.storageSet('musicVolume', ig.music.volume);
        }
    }); MenuItemKeyboardLayout = MenuItem.extend({
        wide: true, init: function () { this.layouts = []; this.current = 0; var selected = ig.storageGet('keyboardLayout'); var i = 0; for (var k in ig.Keyboard.LAYOUT) { var layout = ig.Keyboard.LAYOUT[k]; this.layouts.push({ name: layout.name, layout: layout, key: k }); if (selected === k) { this.current = this.layouts.length - 1; } } }, getText: function () { return this.layouts[this.current].name; }, left: function () {
            this.current--; if (this.current < 0) { this.current = this.layouts.length - 1; }
            if (ig.game && ig.game.keyboard) { ig.game.keyboard.setLayout(this.layouts[this.current].layout); }
        }, right: function () {
            this.current++; if (this.current >= this.layouts.length) { this.current = 0; }
            if (ig.game && ig.game.keyboard) { ig.game.keyboard.setLayout(this.layouts[this.current].layout); }
        }, click: function () {
            if (ig.input.mouse.x > 240) { this.right(); } else { this.left(); }
            ig.storageSet('keyboardLayout', this.layouts[this.current].key);
        }
    }); MenuItemResume = MenuItem.extend({ getText: function () { return 'resume'; }, ok: function () { ig.game.menu = null; } }); MenuSettings = Menu.extend({
        scale: 0.8, clearColor: 'rgba(0,0,0,0.8)', init: function () {
            if (ig.game.mode !== ZType.MODE.GAME) { this.itemClasses.erase(MenuItemResume); }
            ...
            else { this.items[2].y = Math.max(ig.system.height - 220, 490) / this.scale; }
        }, itemClasses: [MenuItemSoundVolume, MenuItemMusicVolume, MenuItemKeyboardLayout, MenuItemResume, MenuItemBack], draw: function () {
            this.width = ig.system.width / this.scale; var ctx = ig.system.context; this.parent(); var s = 0.85; ctx.save(); ctx.scale(s, s); ctx.globalAlpha = 0.5; if (ig.game.mode !== ZType.MODE.GAME) { this.font.draw('SETTINGS', ig.system.width / 2 / s, 60 / s, ig.Font.ALIGN.CENTER); }
...
ig.baked = true; ig.module('game.menus.title').requires('game.menus.base', 'game.menus.settings', 'game.menus.detailed-stats', 'game.ease').defines(function () {
    MenuItemNormalMode = MenuItem.extend({ getText: function () { return 'new game'; }, ok: function () { ig.game.setGame(); }, }); MenuItemSettingsMenu = MenuItem.extend({ getText: function () { return 'settings'; }, ok: function () { ig.game.menu = new MenuSettings(); }, }); MenuItemChangeURL = MenuItem.extend({ getText: function () { return (ig.doc ? 'try a different text' : 'load your own text'); }, ok: function () { document.location.href = '/?load' }, }); MenuItemRemoveAds = MenuItem.extend({ getText: function () { return 'remove ads'; }, ok: function () { ig.game.purchaseRemoveAds(); } }); MenuItemGameCenter = MenuItem.extend({ getText: function () { return 'gamecenter'; }, ok: function () { ig.game.gameCenter.showLeaderboard('score'); } }); MenuItemStats = MenuItem.extend({ getText: function () { return 'my stats'; }, ok: function () { ig.game.detailedStats = ig.game.detailedStats || new DetailedStats(); ig.game.detailedStats.show(); ig.input.delayedKeyup.click = true; } }); MenuItemAppStore = MenuItem.extend({ getText: function () { return 'download as app'; }, ok: function () { window.location.href = ig.ua.iOS ? 'https://goo.gl/qSntlx' : 'https://goo.gl/cg7rmI'; } }); MenuTitle = Menu.extend({
        itemClasses: [MenuItemNormalMode, MenuItemRemoveAds, MenuItemGameCenter], scale: 0.75, y: 0, init: function () {
            this.itemClasses = []; this.itemClasses.push(MenuItemNormalMode); this.itemClasses.push(MenuItemSettingsMenu); if (window.Ejecta && !ig.storageGet('removeAds')) { this.itemClasses.push(MenuItemRemoveAds); }
            ...
            ctx.globalAlpha = Math.max(0.4 - this.transition, 0) + (this.infoIcon.hovered ? 0.3 : 0); this.infoIcon.draw(ig.system.width - 48, ig.system.height - 48); ctx.globalAlpha = 1; ctx.globalCompositeOperation = 'source-over';
        }, update: function () {
            if (ig.game.gameTransitionTimer) { return; }
            if (ig.input.mouse.y < 104 && ig.input.pressed('click')) {
                if (window.ejecta) { ejecta.openURL('https://itunes.apple.com/us/artist/phoboslab/id312666931'); }
                else { window.open('https://itunes.apple.com/us/artist/phoboslab/id312666931'); }
                return;
            }
            this.parent(); if (ig.input.mouse.x > ig.system.width - 64 && ig.input.mouse.y > ig.system.height - 64) {
                if (ig.input.pressed('click')) { ig.game.menu = new MenuAbout(); ig.game.menu.scroll = this.scroll; return; }
...
ig.baked = true; ig.module('impact.entity-pool').requires('impact.game').defines(function () {
    "use strict"; ig.EntityPool = {
        pools: {}, mixin: { staticInstantiate: function (x, y, settings) { return ig.EntityPool.getFromPool(this.classId, x, y, settings); }, erase: function () { ig.EntityPool.putInPool(this); } }, enableFor: function (Class) { Class.inject(this.mixin); }, getFromPool: function (classId, x, y, settings) {
            var pool = this.pools[classId]; if (!pool || !pool.length) { return null; }
            var instance = pool.pop(); instance.reset(x, y, settings); return instance;
        }, putInPool: function (instance) {
            if (!this.pools[instance.classId]) { this.pools[instance.classId] = [instance]; }
            else { this.pools[instance.classId].push(instance); }
        }, drainPool: function (classId) { delete this.pools[classId]; }, drainAllPools: function () { this.pools = {}; }
    }; ig.Game.inject({ loadLevel: function (data) { ig.EntityPool.drainAllPools(); this.parent(data); } });
});

// lib/game/entities/particle.js
ig.baked = true; ig.module('game.entities.particle').requires('impact.entity').defines(function () {
    EntityParticle = ig.Entity.extend({
        size: { x: 4, y: 4 }, offset: { x: 0, y: 0 }, maxVel: { x: 1000, y: 1000 }, minBounceVelocity: 0, type: ig.Entity.TYPE.NONE, checkAgainst: ig.Entity.TYPE.NONE, collides: ig.Entity.COLLIDES.LITE, lifetime: 5, fadetime: 1, bounciness: 0.6, friction: { x: 20, y: 0 }, init: function (x, y, settings) { this.parent(x, y, settings); this.vel.x = (Math.random() * 2 - 1) * this.vel.x; this.vel.y = (Math.random() * 2 - 1) * this.vel.y; this.currentAnim.flip.x = (Math.random() > 0.5); this.currentAnim.flip.y = (Math.random() > 0.5); this.currentAnim.gotoRandomFrame(); this.idleTimer = new ig.Timer(); }, update: function () {
        ...
        }, reset: function (x, y, settings) { this.parent(x, y, settings); this.vel.x = (Math.random() * 2 - 1) * this.vel.x; this.vel.y = (Math.random() * 2 - 1) * this.vel.y; this.currentAnim.flip.x = (Math.random() > 0.5); this.currentAnim.flip.y = (Math.random() > 0.5); this.currentAnim.gotoRandomFrame(); this.idleTimer.reset(); }
    });
});

// lib/game/entities/explosion.js
ig.baked = true; ig.module('game.entities.explosion').requires('impact.entity', 'impact.entity-pool', 'game.entities.particle').defines(function () {
    window.EntityExplosionParticle = EntityParticle.extend({ lifetime: 0.5, fadetime: 0.5, vel: { x: 60, y: 60 }, animSheet: new ig.AnimationSheet('media/sprites/explosion.png', 32, 32), init: function (x, y, settings) { this.addAnim('idle', 5, [0, 1, 2]); this.parent(x, y, settings); }, update: function () { this.currentAnim.angle += 0.1 * ig.system.tick; this.parent(); } }); ig.EntityPool.enableFor(EntityExplosionParticle); window.EntityExplosionSpark = EntityParticle.extend({ lifetime: 0.5, fadetime: 0.5, vel: { x: 60, y: 60 }, animSheet: new ig.AnimationSheet('media/sprites/spark.png', 48, 48), init: function (x, y, settings) { this.addAnim('idle', 1, [0]); this.parent(x, y, settings); }, update: function () { this.currentAnim.angle += 0.1 * ig.system.tick; this.parent(); } }); ig.EntityPool.enableFor(EntityExplosionSpark); window.EntityExplosionHuge = ig.Entity.extend({
        lifetime: 1, fadetime: 1, alpha: 0, img: new ig.Image('media/sprites/explosion-huge.jpg', 512, 512), init: function (x, y, settings) { this.parent(x, y, settings); this.idleTimer = new ig.Timer(); }, update: function () {
            if (this.idleTimer.delta() > this.lifetime) { this.kill(); return; }
            this.alpha = this.idleTimer.delta().map(this.lifetime - this.fadetime, this.lifetime, 1, 0);
        }, draw: function () { var ctx = ig.system.context; ctx.save(); var scale = this.alpha.map(0, 1, 10, 0); ctx.translate(this.pos.x - ig.game._rscreen.x, this.pos.y - ig.game._rscreen.y); ctx.scale(scale, scale); ctx.globalAlpha = this.alpha; this.img.draw(-256, -256); ctx.globalAlpha = 1; ig.system.context.restore(); }
    }); window.EntityExplosionHugeGlitch = EntityExplosionHuge.extend({
        img: new ig.Image('media/sprites/explosion-huge-glitch.jpg', 512, 512), glitchJPEG: new ig.Image('media/title/glitch-jpeg.png'), glitchStripe: new ig.Image('media/title/glitch-stripe.png'), draw: function () {
            this.parent(); var d = this.idleTimer.delta(); if (d < 0.1 || (d > 0.2 && d < 0.3)) { this.glitchJPEG.draw(this.pos.x - 240 + (200 - Math.random() * 400), Math.random() * ig.system.height); }
            if (d < 0.15 || (d > 0.25 && d < 0.35)) { this.glitchStripe.draw(0, Math.random() * ig.system.height); }
        }, kill: function () { this.parent(); ig.game.showGameOverScreen(); }
    });
});

// lib/game/entities/enemy.js
ig.baked = true; ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function () {
    EntityEnemy = ig.Entity.extend({
        word: 'none', remainingWord: 'none', health: 8, currentLetter: 0, targeted: false, font: new ig.FontAvenirNext('media/fonts/avenir-22-white-v2.png'), fontActive: new ig.FontAvenirNext('media/fonts/avenir-22-orange-v2.png'), speed: 10, friction: { x: 100, y: 100 }, hitTimer: null, dead: false, angle: 0, wordLength: { min: 8, max: 8 }, numKillParticles: 20, numHitParticles: 5, soundHit: new ig.Sound('media/sounds/hit.ogg'), soundTarget: new ig.Sound('media/sounds/target.ogg'), reticleTimer: null, reticle: new ig.Image('media/ui/reticle.png'), type: ig.Entity.TYPE.B, checkAgainst: ig.Entity.TYPE.A, init: function (x, y, settings) { this.parent(x, y, settings); this.font.letterSpacing = 0.5; this.fontActive.letterSpacing = 0.5; var length = Math.random().map(0, 1, this.wordLength.min, this.wordLength.max).round(); this.word = settings.word || this.getWordWithLength(length); this.health = this.word.length; this.remainingWord = this.word; this.hitTimer = new ig.Timer(0); this.dieTimer = new ig.Timer(0); ig.game.registerTarget(this.word.charAt(0), this); this.angle = this.angleTo(ig.game.player); if (!ig.ua.mobile) { this.speed *= ig.game.speedFactor; } }, getWordWithLength: function (l) {
            var w = 'wtf'; for (var i = 0; i < 20; i++) {
                if (l >= 2 && l <= 12) { w = ig.game.wordlist[l].random(); }
            ...
            else { this.font.draw(this.remainingWord, x + 2, y + 6, ig.Font.ALIGN.RIGHT); }
        }, kill: function (silent) {
            if (this.remainingWord.length) { ig.game.unregisterTarget(this.remainingWord.charAt(0), this); }
            ...
            if (!silent) { ig.game.lastKillTimer.set(0.3); var px = this.pos.x - this.size.x / 2, py = this.pos.y - this.size.y / 2; for (var i = 0; i < this.wordLength.max; i++) { ig.game.spawnEntity(EntityExplosionSpark, px, py, { vel: { x: 360, y: 360 } }); } }
        }, cancel: function () { ig.game.currentTarget = null; this.targeted = false; ig.game.registerTarget(this.remainingWord.charAt(0), this); }, update: function () {
            if (this.hitTimer.delta() > 0) { this.vel.x = Math.cos(this.angle) * this.speed; this.vel.y = Math.sin(this.angle) * this.speed; }
            ...
            if (this.reticleTimer && this.reticleTimer.delta() > 0) { this.reticleTimer = null; }
        }, hit: function (other) { this.spawnExplosionParticles((this.health <= 1)); this.vel.x = -Math.cos(this.angle) * 20 + Math.random() * 20; this.vel.y = -Math.sin(this.angle) * 20 + Math.random() * 20; this.hitTimer.set(0.3); ig.game.lastKillTimer.set(0.2); this.receiveDamage(1); this.soundHit.play(); }, spawnExplosionParticles: function (killed) {
            var numParticles = killed ? this.numKillParticles : this.numHitParticles; for (var i = 0; i < numParticles; i++) { ig.game.spawnEntity(EntityExplosionParticle, this.pos.x, this.pos.y); }
            var numSparks = numParticles / 2; for (var i = 0; i < numSparks; i++) { ig.game.spawnEntity(EntityExplosionSpark, this.pos.x, this.pos.y); }
        }, isHitBy: function (letter) {
            var expected = ig.game.translateUmlaut(this.remainingWord.charAt(0)); if (!ig.game.textIsCaseSensitive) { expected = expected.toLowerCase(); letter = letter.toLowerCase(); }
            if (expected == letter) {
                this.remainingWord = this.remainingWord.substr(1); if (this.remainingWord.length == 0) { ig.game.currentTarget = null; this.dead = true; }
                return true;
            }
            else { return false; }
        }, check: function (other) { other.kill(); this.kill(); }
    });
});

// lib/game/entities/enemy-missle.js
ig.baked = true; ig.module('game.entities.enemy-missle').requires('game.entities.enemy').defines(function () {
    EntityEnemyMissle = EntityEnemy.extend({
        size: { x: 8, y: 15 }, offset: { x: 6, y: 7 }, animSheet: new ig.AnimationSheet('media/sprites/missle.png', 20, 26), explosionSound: new ig.Sound('media/sounds/explosion-small.ogg'), health: 4, speed: 35, targetTimer: null, wordLength: { min: 2, max: 5 }, numKillParticles: 10, numHitParticles: 2, init: function (x, y, settings) {
            if (ig.doc) { this.wordLength.min = 1; this.wordLength.max = 1; }
            this.parent(x, y, settings); this.addAnim('idle', 1, [0]); this.angle = settings.angle; this.currentAnim.angle = this.angle - Math.PI / 2; this.targetTimer = new ig.Timer(1);
        }, update: function () {
            var d = this.targetTimer.delta(); if (d > 0 && d < 0.7) { var ad = this.angle - this.angleTo(ig.game.player); this.angle -= ad * ig.system.tick * 2; this.currentAnim.angle = this.angle - Math.PI / 2; }
            this.parent();
        }
    });
});
...
ig.baked = true; ig.module('game.entities.enemy-destroyer').requires('game.entities.enemy', 'game.entities.explosion').defines(function () {
    EntityEnemyDestroyer = EntityEnemy.extend({
        size: { x: 24, y: 34 }, offset: { x: 10, y: 8 }, animSheet: new ig.AnimationSheet('media/sprites/destroyer.png', 43, 58), explosionSound: new ig.Sound('media/sounds/explosion-large.ogg'), health: 8, speed: 20, shootTimer: null, wordLength: { min: 7, max: 10 }, numKillParticles: 40, numHitParticles: 10, init: function (x, y, settings) {
            this.parent(x, y, settings); this.addAnim('idle', 1, [0]); this.shootTimer = new ig.Timer(ig.doc ? 12 : 5); this.angle = (Math.random().map(0, 1, 67, 90)
                + (this.pos.x > ig.system.width / 2 ? 22.5 : 0)) * Math.PI / 180; this.currentAnim.angle = this.angle - Math.PI / 2;
        }, kill: function (silent) { this.parent(silent); if (!silent) { ig.game.spawnEntity(EntityExplosionHuge, this.pos.x + 12, this.pos.y + 22); ig.game.screenShake(10); } }, update: function () { this.parent(); if (this.shootTimer.delta() > 0) { this.shootTimer.reset(); if (this.distanceTo(ig.game.player) > 100) { ig.game.spawnEntity(EntityEnemyMissle, this.pos.x + 12, this.pos.y + 22, { angle: this.angle }); } } }
    });
});

// lib/game/entities/enemy-oppressor.js
ig.baked = true; ig.module('game.entities.enemy-oppressor').requires('game.entities.enemy').defines(function () {
    EntityEnemyOppressor = EntityEnemy.extend({
        size: { x: 36, y: 58 }, offset: { x: 16, y: 10 }, animSheet: new ig.AnimationSheet('media/sprites/oppressor.png', 68, 88), explosionSound: new ig.Sound('media/sounds/explosion-large.ogg'), health: 10, speed: 15, shootTimer: null, bullets: 8, wordLength: { min: 9, max: 12 }, numKillParticles: 40, numHitParticles: 10, init: function (x, y, settings) { this.parent(x, y - 18, settings); this.addAnim('idle', 1, [0]); this.shootTimer = new ig.Timer(ig.doc ? 14 : 7); this.angle = Math.PI / 2; }, kill: function (silent) { this.parent(silent); if (!silent) { ig.game.spawnEntity(EntityExplosionHuge, this.pos.x + 12, this.pos.y + 22); ig.game.screenShake(20); } }, update: function () {
            this.parent(); if (this.shootTimer.delta() > 0) {
                if (this.distanceTo(ig.game.player) > 100) { var inc = 100 / (this.bullets - 1); var a = 40; var radius = 21; for (var i = 0; i < this.bullets; i++) { var angle = a * Math.PI / 180; var x = this.pos.x + 18 + Math.cos(angle) * radius; var y = this.pos.y + 48 + Math.sin(angle) * radius; ig.game.spawnEntity(EntityEnemyBullet, x, y, { angle: angle }); a += inc; } }
                this.shootTimer.reset();
            }
        }
    }); EntityEnemyBullet = EntityEnemy.extend({ size: { x: 2, y: 2 }, offset: { x: 8, y: 11 }, animSheet: new ig.AnimationSheet('media/sprites/bullet.png', 20, 24), health: 1, speed: 50, wordLength: { min: 1, max: 1 }, init: function (x, y, settings) { this.parent(x, y, settings); this.addAnim('idle', 1, [0]); this.angle = settings.angle; this.currentAnim.angle = this.angle - Math.PI / 2; } });
});

// lib/game/entities/emp.js
ig.baked = true; ig.module('game.entities.emp').requires('impact.entity').defines(function () {
    "use strict"; window.EntityEMP = ig.Entity.extend({
        lifetime: 1, fadetime: 1, alpha: 0, killedProjectiles: false, img: new ig.Image('media/sprites/emp.png'), sound: new ig.Sound('media/sounds/emp.ogg', false), init: function (x, y, settings) { this.parent(x, y, settings); this.idleTimer = new ig.Timer(); this.sound.play(); ig.game.screenShake(30); }, update: function () {
            this.alpha = this.idleTimer.delta().map(this.lifetime - this.fadetime, this.lifetime, 1, 0); var scale = this.alpha.map(0, 1, 1.7, 0); scale = Math.pow(scale, 0.5) * 200; var ents = ig.game.entities; for (var i = 0; i < ents.length; i++) { var ent = ents[i]; if (ent instanceof EntityEnemy) { if (this.distanceTo(ent) < scale) { ent.spawnExplosionParticles(true); ent.kill(); } } }
            if (this.idleTimer.delta() > this.lifetime) { this.kill(); return; }
        }, draw: function () { var scale = Math.pow(this.alpha.map(0, 1, 1.7, 0), 0.5); var ctx = ig.system.context; ctx.save(); ctx.translate(this.pos.x - ig.game._rscreen.x, this.pos.y - ig.game._rscreen.y); ctx.scale(scale * 2, scale * 2); ctx.rotate(Math.pow(scale, 0.5) * Math.PI * 2); ctx.globalAlpha = this.alpha; this.img.draw(-128, -128); ig.system.context.restore(); }
    });
});

// lib/game/entities/player.js
ig.baked = true; ig.module('game.entities.player').requires('impact.entity', 'impact.entity-pool', 'game.entities.emp', 'game.entities.particle').defines(function () {
    EntityPlayer = ig.Entity.extend({
        animSheet: new ig.AnimationSheet('media/sprites/ship.png', 24, 24), targetAngle: 0, size: { x: 8, y: 8 }, offset: { x: 8, y: 8 }, angle: 0, targetAngle: 0, soundShoot: new ig.Sound('media/sounds/plasma.ogg'), soundMiss: new ig.Sound('media/sounds/click.ogg'), soundExplode: new ig.Sound('media/sounds/explosion-player.ogg'), type: ig.Entity.TYPE.A, init: function (x, y, settings) { this.parent(x, y, settings); this.addAnim('idle', 60, [0]); this.addAnim('shoot', 0.05, [3, 2, 1, 0], true); this.addAnim('miss', 0.05, [4, 5, 6], true); }, update: function () {
            ...
            this.currentAnim.angle = this.angle; this.parent();
        }, spawnEMP: function () { if (ig.game.emps) { ig.game.spawnEntity(EntityEMP, this.pos.x + this.size.x / 2, this.pos.y + this.size.y / 2); ig.game.emps--; } }, kill: function () { ig.game.setGameOver(); this.soundExplode.play(); ig.game.spawnEntity(EntityExplosionHugeGlitch, this.pos.x + 12, this.pos.y + 22); ig.game.screenShake(10); this.pos.y = ig.system.height + 300; this.parent(); }, shoot: function (target) { this.currentAnim = this.anims.shoot.rewind(); var ent = ig.game.spawnEntity(EntityPlasma, this.pos.x + 6, this.pos.y + 4); ent.target = target; var angle = this.angleTo(target); this.targetAngle = angle + Math.PI / 2; this.soundShoot.play(); }, miss: function () { this.currentAnim = this.anims.miss.rewind(); this.soundMiss.play(); }
    }); EntityPlasma = ig.Entity.extend({
        speed: 800, maxVel: { x: 10000, y: 10000 }, animSheet: new ig.AnimationSheet('media/sprites/plasma.png', 96, 96), size: { x: 4, y: 4 }, offset: { x: 46, y: 46 }, distance: 100000, init: function (x, y, settings) { this.parent(x, y, settings); this.addAnim('idle', 1, [0]); }, reset: function (x, y, settings) { this.parent(x, y, settings); this.distance = 100000; }, update: function () {
            if (this.target) {
                var currentDistance = this.distanceTo(this.target); if (currentDistance > this.distance || currentDistance < this.target.size.y) { this.target.hit(this); this.kill(); return; }
...
ig.baked = true; ig.module('game.keyboard').requires('impact.font').defines(function () {
    ig.Keyboard = ig.Class.extend({
        background: new ig.Image('media/ui/keyboard-background.png'), keyboard: null, empButton: new ig.Image('media/ui/emp-buttons.png'), pauseButton: new ig.Image('media/ui/pause.png'), font: new ig.Font('media/fonts/avenir-36-blue.png'), hoverImages: { leftEdge: new ig.Image('media/ui/key-edge-left.png'), rightEdge: new ig.Image('media/ui/key-edge-right.png'), normal: new ig.Image('media/ui/key.png') }, multiBar: new ig.Image('media/ui/bar-blue.png'), multiIndicator: new ig.Image('media/ui/multi-indicator.png'), multiSounds: { 2: new ig.Sound('media/sounds/multi-2.ogg'), 3: new ig.Sound('media/sounds/multi-3.ogg') }, width: 0, height: 0, hoverKey: null, expectedKeys: null, keyMetrics: { width: 64, height: 106, offsetTop: 32 }, init: function (callback, layout) { this.callback = callback; this.unsetHoverKeyTimer = null; this.width = this.background.width; this.height = (ig.ua.mobile ? this.background.height : 64); this.drawScale = ig.system.width / this.width; this.x = 0; this.y = ig.system.height / this.drawScale - this.height; if (ig.ua.mobile) { ig.system.canvas.addEventListener('touchstart', this.touchMove.bind(this), false); ig.system.canvas.addEventListener('touchmove', this.touchMove.bind(this), false); ig.system.canvas.addEventListener('touchend', this.touchEnd.bind(this), false); this.keyboardCanvas = ig.$new('canvas'); this.keyboardCanvas.width = this.background.width; this.keyboardCanvas.height = this.background.height; this.keyboardContext = this.keyboardCanvas.getContext('2d'); this.setLayout(layout || ig.Keyboard.LAYOUT.QWERTY); } }, setLayout: function (layout) {
            ...
            return key;
        }, touchMove: function (ev) { var key = this.getCurrentKey(ev.touches); if (key !== 'ESC' && key !== 'ENTER' && key !== 'BACKSPACE') { this.hoverKey = key; this.unsetHoverKeyTimer = null; } }, touchEnd: function (ev) {
            var key = this.getCurrentKey(ev.changedTouches); if (key) { this.callback(key); }
            this.unsetHoverKeyTimer = new ig.Timer();
        }, showMultiplier: function (m) { this.multiplierTimer = new ig.Timer(2); this.multiplierIndex = m - 2; this.multiSounds[m].play(); }, draw: function () {
            if (ig.ua.mobile) { this.drawFull(); }
            else { this.drawMinimal(); }
        }, drawFull: function () {
            var ctx = ig.system.context; ctx.save(); ctx.globalAlpha = 1; ctx.scale(this.drawScale, this.drawScale); ctx.drawImage(this.keyboardCanvas, this.x, this.y); this.drawMultiplierBar(this.x, this.y); if (this.unsetHoverKeyTimer && this.unsetHoverKeyTimer.delta() > 0.25) { this.hoverKey = null; this.unsetHoverKeyTimer = null; }
            if (this.hoverKey) {
                var offsetX = 0; var hoverImage = this.hoverImages.normal; var kpos = this.keyPositions[this.hoverKey]; if (kpos[0] < 0.5) { hoverImage = this.hoverImages.leftEdge; offsetX = 26; }
            ...
            this.empButton.drawTile(this.x + 582, this.y + 250, ig.game.emps, 34, 40); ctx.globalAlpha = 0.15; this.pauseButton.draw(28, 28); ctx.restore();
        }, drawMinimal: function () {
            var ctx = ig.system.context; ctx.save(); ctx.globalAlpha = 1; ctx.scale(this.drawScale, this.drawScale); this.drawMultiplierBar(this.x, this.y + 62); if (!ig.game.emps) { ctx.globalAlpha = 0.7; }
            this.empButton.drawTile(this.x + 582, this.y, ig.game.emps, 34, 40); ctx.globalAlpha = 0.1; this.pauseButton.draw(28, 28); ctx.restore();
        }, drawMultiplierBar: function (x, y) {
            if (ig.game.streak > 0) { var multiLength = 1 - (75 / (75 + ig.game.streak * 2)); this.multiBar.draw(x, y, 0, 0, this.width * multiLength, 2); }
            var showMultiplierTime = this.multiplierTimer ? this.multiplierTimer.delta() : 0; if (showMultiplierTime < 0) { ig.system.context.globalAlpha = showMultiplierTime.map(-2, 0, 2, 0).limit(0, 1); this.multiIndicator.drawTile(x + this.width * multiLength - 30, y - 22, this.multiplierIndex, 32, 18); ig.system.context.globalAlpha = 1; }
        }
    }); ig.Keyboard.LAYOUT = {}; ig.Keyboard.LAYOUT.QWERTY = { name: 'QWERTY (us, uk)', map: [['q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p'], ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'], ['z', 'x', 'c', 'v', 'b', 'n', 'm']] }; ig.Keyboard.LAYOUT.QWERTZ = { name: 'QWERTZ (de, at, cz)', map: [['q', 'w', 'e', 'r', 't', 'z', 'u', 'i', 'o', 'p'], ['a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l'], ['y', 'x', 'c', 'v', 'b', 'n', 'm']] }; ig.Keyboard.LAYOUT.AZERTY = { name: 'AZERTY (fr, be)', map: [['a', 'z', 'e', 'r', 't', 'w', 'u', 'i', 'o', 'p'], ['q', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm'], ['y', 'x', 'c', 'v', 'b', 'n']] }; ig.Keyboard.LAYOUT.COLEMAK = { name: 'Colemak', map: [['q', 'w', 'f', 'p', 'g', 'j', 'l', 'u', 'y'], ['a', 'r', 's', 't', 'd', 'h', 'n', 'e', 'i', 'o'], ['z', 'x', 'c', 'v', 'b', 'k', 'm']] }; ig.Keyboard.LAYOUT.DVORAK = { name: 'Dvorak', map: [['p', 'y', 'f', 'g', 'c', 'r', 'l'], ['a', 'o', 'e', 'u', 'i', 'd', 'h', 't', 'n', 's'], ['q', 'j', 'k', 'x', 'b', 'm', 'w', 'v', 'z', '']] };
});

// lib/plugins/silent-loader.js
ig.baked = true; ig.module('plugins.silent-loader').requires('impact.loader').defines(function () {
    ig.SilentLoader = ig.Loader.extend({
        endTimer: 0, fadeToGameTime: 300, end: function () { this.parent(); this.endTime = Date.now(); ig.system.setDelegate(this); }, run: function () {
            var t = Date.now() - this.endTime; var alpha = 1; if (t < this.fadeToGameTime) { ig.game.run(); alpha = t.map(0, this.fadeToGameTime, 1, 0); }
...
ig.baked = true; ig.module('plugins.rise-loader').requires('impact.loader').defines(function () {
...
ig.baked = true; ig.module('game.document-scanner').defines(function () {
    "use strict"; ig.chunkify = function (a, n, balanced) {
        if (n < 2) { return [a]; }
        var len = a.length, out = [], i = 0, size; if (len % n === 0) { size = Math.floor(len / n); while (i < len) { out.push(a.slice(i, i += size)); } }
            ...
            n--; size = Math.floor(len / n); if (len % size === 0) { size--; }
    ...
    ig.DocumentScanner = ig.Class.extend({
        _reSkipNode: null, _reSkipClass: null, _reTextNode: null, _reHasWords: null, _reSplitSentences: null, _currentFragment: null, _animationDoneCallback: null, _hurry: false, _scrollAnimationId: 0, looksLikeEnglish: false, fragments: [], highlightMarginTop: 32, highlightClass: 'ztype-current-text-fragment', init: function (rootElement, enableNumbers, enablePunctionations) {
            this._reSkipNode = /^(script|style|head|link|meta|button|input|nav|label|img|svg|canvas|ins)$/i; this._reSkipClass = /head|mw-jump-link|foot|links|menu|nav|pagetop|subtext|side|notice|overlay|siteSub|infobox|hatnote|abstract|caption|disclaimer|byline/i; this._reTextNode = /^(em|strong|b|i|mark|cite|dfn|small|del|ins|sub|sup|a|#text|span|code)$/i; this._reHasWords = /\b[^\d\W]{2,}\b/g; this._reSplitSentences = /([.?!:\/]+[\s\[\]]+)(?=\w)/g; this._reHasEnglishWords = /\b(the|be|to|of|and|that|have|with|you|this|say)\b/i; if (enableNumbers) { this._reHasWords = /\b\w{2,}\b/g; }
            if (enablePunctionations) { this._reHasWords = /[^\s]+/g; }
            this.$scanOverlay = ig.$('#ztype-overlay'); this.$scanProgress = ig.$('#ztype-scan-progress'); this.$scanOverlay.className = 'ztype-scanning'; var elements = this.traverse(rootElement); this.fragments = []; for (var i = 0; i < elements.length; i++) { this.splitIntoFragments(elements[i], this.fragments, elements.length); }
            ...
            var allChildrenAreLinks = element.childNodes.length > 0; for (var i = 0; i < element.childNodes.length; i++) {
            ...
            var parent = element.parentNode; var text = element.textContent; var sentences = text.replace(this._reSplitSentences, '$1%%ZT%%').split('%%ZT%%'); if (totalElements === 1 && sentences.length < 2) {
                var words = text.trim().split(/\s+/); var wordCount = words.length; var sentenceLength = 2; if (wordCount > 30) { sentenceLength = 10; }
            ...
            if (element.nodeName === '#text') {
                for (var i = 0; i < spans.length; i++) {
                    if (i > 0) { parent.insertBefore(spans[i], spans[i - 1].nextSibling); }
                    else { parent.replaceChild(spans[i], element); }
                }
            }
            else { element.innerHTML = ''; for (var i = 0; i < spans.length; i++) { element.appendChild(spans[i]); } }
        }, childrenAreText: function (element) {
            for (var i = 0; i < element.childNodes.length; i++) { if (!element.childNodes[i].nodeName.match(this._reTextNode) || (element.id && element.id.match(this._reSkipClass)) || (element.className && element.className.match(this._reSkipClass)) || !this.childrenAreText(element.childNodes[i])) { return false; } }
            return true;
        }, _detectEnglishText: function () {
            var count = 0; for (var i = 0; i < this.fragments.length; i++) { if (this.fragments[i].text.match(this._reHasEnglishWords)) { count++; } }
            this.looksLikeEnglish = count > (this.fragments.length / 4); this._lleCount = count;
        }, highlightFragment: function (fragment, margin, dontDeHighlight) {
            if (this._currentFragment && !dontDeHighlight) { this._currentFragment.className = ''; }
            this._currentFragment = fragment.element; if (this._currentFragment) {
                this._currentFragment.className = this.highlightClass; var target = (document.documentElement.scrollTop || document.body.scrollTop)
                    + this._currentFragment.getBoundingClientRect().top - (margin || this.highlightMarginTop); this.scrollTo(target);
            }
        }, scrollTo: function (y) { this._scrollTarget = y; if (!this._scrollAnimationId) { this._scrollAnimationId = setInterval(this._animateScroll.bind(this), 5); } }, _animateScroll: function () {
            var current = document.documentElement.scrollTop = document.body.scrollTop; var diff = (this._scrollTarget - current); var pos = current + diff / 10; if (Math.abs(diff) > 1024) { pos = this._scrollTarget; }
            document.documentElement.scrollTop = document.body.scrollTop = pos; if (Math.abs(diff) < 10) { clearInterval(this._scrollAnimationId); this._scrollAnimationId = 0; }
        }, playScanAnimation: function (doneCallback) { setTimeout(this.nextAnimationStep.bind(this, 0), 300); this._animationDoneCallback = doneCallback; }, fastForwardScanAnimation: function () { this._hurry = true; }, nextAnimationStep: function (current) {
            if (this.fragments[current]) {
                this.highlightFragment(this.fragments[current], window.innerHeight / 3, true); var cc = current + 1; if (this._hurry) { cc = cc * cc; }
            ...
            this._currentFragment = null; this.$scanProgress.style.width = '100%'; this.$scanOverlay.className = 'ztype-scan-done'; this._detectEnglishText(); setTimeout((function () {
                this.scrollTo(0); for (var i = 0; i < this.fragments.length; i++) { this.fragments[i].element.className = ''; }
                this.$scanOverlay.className = 'ztype-playing'; if (this._animationDoneCallback) { this._animationDoneCallback(); }
            }).bind(this), 300);
        }
    });
});
...
ig.baked = true; ig.module('game.main').requires('impact.game', 'impact.font', 'game.menus.about', 'game.menus.game-over', 'game.menus.settings', 'game.menus.title', 'game.entities.enemy-missle', 'game.entities.enemy-mine', 'game.entities.enemy-destroyer', 'game.entities.enemy-oppressor', 'game.entities.player', 'game.keyboard', 'game.xhr', 'game.ease', 'plugins.silent-loader', 'plugins.rise-loader', 'game.document-scanner', 'game.words.en').defines(function () {
    Number.zeroes = '000000000000'; Number.prototype.zeroFill = function (d) { var s = this.toString(); return Number.zeroes.substr(0, d - s.length) + s; }; ig.queryParam = function (name) {
        ...
        var value = null; try { value = localStorage.getItem(key); }
        catch (err) { }
        return value;
        ...
        font: new ig.Font('media/fonts/avenir-18-white.png'), fontTitle: new ig.Font('media/fonts/avenir-36-blue.png'), separatorBar: new ig.Image('media/ui/bar-blue.png'), idleTimer: null, spawnTimer: null, targets: {}, currentTarget: null, yScroll: 0, yScroll2: 0, gradient: new ig.Image('media/background/gradient.png'), stars: new ig.Image('media/background/stars.jpg'), grid: new ig.Image('media/background/grid.png'), music1: new ig.Sound('media/music/endure.ogg', false), music2: new ig.Sound('media/music/orientation.ogg', false), cancelSound: new ig.Sound('media/sounds/cancel.ogg'), spawnSound: new ig.Sound('media/sounds/spawn.ogg'), menu: null, mode: 0, score: 0, streak: 0, hits: 0, misses: 0, multiplier: 1, wave: {}, gameTime: 0, kills: 0, emps: 0, personalBest: 0, isPersonalBest: false, waitingForItunes: false, adPage: null, difficulty: (ig.ua.mobile ? 'MOBILE' : 'DESKTOP'), keyboard: null, textIsCaseSensitive: !!document.location.hash.match('cs'), textHasPunctuations: false, textHasNumbers: false, _screenShake: 0, wordlist: null, init: function () {
            var flags = parseInt(ig.queryParam('flags')) || window.ZTYPE_FLAGS; if (flags & ZType.FLAG.CASE_SENSITIVE) { this.textIsCaseSensitive = true; }
            ...
            this.setTitle(); this.wordlist = ig.WORDS.EN; if (ig.doc) {
                var alpha = 'a-zßàáâãäåæçèéêëìíîïðñòóôõöøùúûüý'; var num = '0-9'; var punct = '!"#$%&\'()*+,\\-\\.\\/:;<=>?@\\[\\]\\\\\\^_`{|}~'; var chars = alpha; if (this.textHasNumbers) { chars += num; }
            ...
            this.bufferedLetters = []; var openFeedbackForm = document.getElementById('open-feedback-form'); if (openFeedbackForm) { openFeedbackForm.addEventListener('click', this.openFeedbackForm.bind(this)); }
        }, reset: function () { this.entities = []; this.currentTarget = null; this.bufferedLetters = []; this.wave = ig.copy(ZType.WAVES[this.difficulty]); this.score = 0; this.rs = 0; this.streak = 0; this.longestStreak = 0; this.hits = 0; this.misses = 0; this.kills = 0; this.multiplier = 1; this.gameTime = 0; this.isPersonalBest = false; this.speedFactor = 1; this.lastKillTimer = new ig.Timer(); this.spawnTimer = new ig.Timer(); this.idleTimer = new ig.Timer(); this.waveEndTimer = null; }, nextWave: function () {
            this.wave.wave++; this.wave.spawnWait = (this.wave.spawnWait * 0.97).limit(0.2, 1); this.wave.currentSpawnWait = this.wave.spawnWait; this.wave.spawn = []; this.speedFactor *= this.wave.speedIncrease; if (ig.doc) {
                for (var i = 0; i < 10 && this.wave.spawn.length < 2; i++) { this.nextDocFragment(); }
                this.wave.spawn.reverse();
            }
            else {
                var dec = 0; for (var t = 0; t < this.wave.types.length; t++) {
                    var type = this.wave.types[t]; type.count -= dec; if (this.wave.wave % type.incEvery == 0) { type.count++; dec++; }
            ...
            this.wave.fragment++; var fragment = ig.doc.fragments[(this.wave.fragment - 1) % ig.doc.fragments.length]; if (!ig.ua.mobile) { ig.doc.highlightFragment(fragment); }
            ...
            var wordsByLength = filteredWords.slice().sort(function (a, b) { return b.length - a.length; }); var bigShipChance = (this.wave.types[0].count + this.wave.types[1].count) / this.wave.types[2].count; var wordLengthForBigShip = wordsByLength[Math.floor(wordsByLength.length * bigShipChance * 0.75)].length; var longSentenceFactor = (filteredWords.length / 8).limit(1, 1.5); for (var i = 0; i < filteredWords.length; i++) { var w = filteredWords[i]; var wait = (w.length / 5).limit(0.7, 3) * 1.2 * longSentenceFactor; var type = (w.length > wordLengthForBigShip) ? (Math.random() > 0.75 ? EntityEnemyOppressor : EntityEnemyDestroyer) : EntityEnemyMine; this.wave.spawn.push({ type: type, word: w, wait: wait }); }
        }, spawnCurrentWave: function () {
            if (!this.wave.spawn.length) {
                if (this.entities.length <= 1 && !this.waveEndTimer) { this.waveEndTimer = new ig.Timer(2); }
                else if (this.waveEndTimer && this.waveEndTimer.delta() > 0) { this.waveEndTimer = null; this.nextWave(); }
            }
            else if (this.spawnTimer.delta() > this.wave.currentSpawnWait) { this.spawnTimer.reset(); var spawn = this.wave.spawn.pop(); var x = Math.random().map(0, 1, 10, ig.system.width - 10); var y = -30; this.spawnEntity(spawn.type, x, y, { healthBoost: this.wave.healthBoost, word: spawn.word }, true); this.wave.currentSpawnWait = spawn.wait ? this.wave.spawnWait * spawn.wait : this.wave.spawnWait; }
        }, spawnEntity: function (type, x, y, settings, atBeginning) {
            var ent = new (type)(x, y, settings || {}); if (atBeginning) { this.entities.unshift(ent); }
            else { this.entities.push(ent); }
            if (ent.name) { this.namedEntities[ent.name] = ent; }
            return ent;
        }, registerTarget: function (letter, ent) {
            var c = this.translateUmlaut(this.prepareLetter(letter)); if (this.targets[c] === undefined) { this.targets[c] = [ent]; }
            else { this.targets[c].push(ent); }
            if (!this.currentTarget) { this.setExpectedKeys(); }
        }, unregisterTarget: function (letter, ent) {
            var c = this.translateUmlaut(this.prepareLetter(letter)); if (this.targets[c] !== undefined) { this.targets[c].erase(ent); }
            if (!this.currentTarget) { this.setExpectedKeys(); }
        }, setExpectedKeys: function () { this.keyboard.expectedKeys = []; for (var k in this.targets) { if (this.targets[k].length) { this.keyboard.expectedKeys.push(k); } } }, _umlautTable: { 'ß': 's', 'à': 'a', 'á': 'a', 'â': 'a', 'ã': 'a', 'ä': 'a', 'å': 'a', 'æ': 'a', 'ç': 'c', 'è': 'e', 'é': 'e', 'ê': 'e', 'ë': 'e', 'ì': 'i', 'í': 'i', 'î': 'i', 'ï': 'i', 'ð': 'd', 'ñ': 'n', 'ò': 'o', 'ó': 'o', 'ô': 'o', 'õ': 'o', 'ö': 'o', 'ø': 'o', 'ù': 'u', 'ú': 'u', 'û': 'u', 'ü': 'u', 'ý': 'y' }, translateUmlaut: function (k) {
            if (ig.ua.mobile || (ig.doc && ig.doc.looksLikeEnglish)) { return this._umlautTable[k] || k; }
            else { return k; }
        }, prepareLetter: function (c) {
            if (this.textIsCaseSensitive) { return c; }
            else { return c.toLowerCase(); }
        }, keypress: function (ev) {
            if (ev.target.tagName == 'INPUT' || ev.ctrlKey || this.mode != ZType.MODE.GAME || this.menu) { return true; }
            var c = ev.charCode; if (c < 33) { return true; }
            ev.stopPropagation(); ev.preventDefault(); var letter = this.prepareLetter(String.fromCharCode(c)); this.bufferLetter(letter); return false;
        }, keydown: function (ev) {
            if (ev.target.tagName == 'INPUT' || ev.ctrlKey || ev.altKey || this.mode != ZType.MODE.GAME || this.menu) { return true; }
            var c = ev.which; if (c === ig.KEY.ENTER) { this.player.spawnEMP(); return false; }
            if (c == ig.KEY.BACKSPACE) {
                if (this.currentTarget) { this.currentTarget.cancel(); this.cancelSound.play(); }
                ev.preventDefault(); return false;
            }
            return true;
        }, virtualKeydown: function (letter) {
            if (this.mode != ZType.MODE.GAME || this.menu) { return true; }
        ...
        }, bufferLetter(letter) { this.bufferedLetters.push(letter); }, shootBufferedLetters: function () {
            while (this.bufferedLetters.length > 0) {
                var letter = this.bufferedLetters.shift(); if (!this.currentTarget) {
                    var potentialTargets = this.targets[letter]; var nearestDistance = -1; var bestTarget = null; if (potentialTargets && potentialTargets.length === 1) { bestTarget = potentialTargets[0]; }
                    else if (potentialTargets && potentialTargets.length > 1) {
                        for (var i = 0; i < this.bufferedLetters.length; i++) {
                            var bufferedLetter = this.bufferedLetters[i]; var remainingPotentialTargets = []; for (var j = 0; j < potentialTargets.length; j++) { var enemy = potentialTargets[j]; if (enemy.remainingWord.charAt(i + 1) === bufferedLetter) { remainingPotentialTargets.push(enemy); } }
            ...
            this.idleTimer.reset(); if (this.currentTarget) {
                var target = this.currentTarget; var hit = this.currentTarget.isHitBy(letter); if (hit) {
                    this.player.shoot(target); this.score += this.multiplier; this.hits++; this.streak++; this.longestStreak = Math.max(this.streak, this.longestStreak); if (ZType.MULTIPLIER_TIERS[this.streak]) { this.multiplier = ZType.MULTIPLIER_TIERS[this.streak]; this.keyboard.showMultiplier(this.multiplier); }
        ...
        }, setGame: function () { this.setGameState(); this.gameTransitionTimer = new ig.Timer(2); this.nextWave(); }, setGameState: function () { this.reset(); var sx = ig.system.width / 2 - 6, sy = ig.system.height - this.keyboard.height * this.keyboard.drawScale - 30; this.player = this.spawnEntity(EntityPlayer, sx, sy); this.mode = ZType.MODE.GAME; ig.music.next(); this.spawnSound.play(); this.emps = 3; }, continue: function () { var fragment = this.wave.fragment; this.setGameState(); this.wave.fragment = Math.max(0, fragment - 1); this.nextWave(); this.menu = null; }, setGameOver: function () {
            if (this.score > this.personalBest) { this.isPersonalBest = true; this.personalBest = this.score; ig.storageSet('highscore', this.personalBest); }
            if (this.gameCenter && this.score > 5) { this.gameCenter.reportScore('score', this.score); }
            this.mode = ZType.MODE.GAME_OVER; ig.music.fadeOut(1);
        }, showGameOverScreen: function () {
            this.menu = new MenuGameOver(); if (this.adPage && !ig.storageGet('removeAds')) { this.adPage.show(); }
            if (this.cocoonInterstitial) { this.cocoonInterstitial.show(); }
        }, setTitle: function () {
            if (this.adPage) { setTimeout(function () { ig.game.adPage.load(); }, 3000); }
            if (this.cocoonInterstitial) { this.cocoonInterstitial.load(); }
            this.reset(); this.mode = ZType.MODE.TITLE; this.menu = new MenuTitle(); this.emps = 0;
        }, update: function () {
            if (ig.input.pressed('menu')) {
                if (this.menu && this.menu instanceof MenuSettings) { this.menu = null; }
                else if (!this.menu) { this.menu = new MenuSettings(); }
            }
            if (this.menu) {
                this.backgroundMaps[0].scroll.y -= 100 * ig.system.tick; if (this.waitingForItunes) { return; }
            ...
            this.parent(); if (this.mode === ZType.MODE.GAME) { this.shootBufferedLetters(); this.spawnCurrentWave(); if (!this.menu && !ig.ua.mobile && ig.input.pressed('click') && ig.input.mouse.x < 64 && ig.input.mouse.y < 64) { this.menu = new MenuSettings(); } }
                ...
                if (this.mode === ZType.MODE.TITLE) { this.setGame(); }
            ...
            this._rscreen.x = Math.random() * this._screenShake; this._rscreen.y = Math.random() * this._screenShake;
        }, screenShake: function (strength) { this._screenShake = Math.max(strength, this._screenShake); }, draw: function () {
            if (this.mode == ZType.MODE.GAME || this.mode === ZType.MODE.GAME_OVER) { this.drawGame(); }
            if (this.menu) {
                this.menu.draw(); if (typeof (this.menu.scroll) != 'undefined') { this.yScroll2 = this.menu.scroll; }
            ...
            if (this.mode == ZType.MODE.GAME) { this.drawUI(); }
        }, drawUI: function () {
            if (this.waveEndTimer) { var d = -this.waveEndTimer.delta(); var a = d > 1.7 ? d.map(2, 1.7, 0, 1) : d < 1 ? d.map(1, 0, 1, 0) : 1; var ys = 276 + (d < 1 ? Math.cos(1 - d).map(1, 0, 0, 250) : 0); var w = this.wave.wave.zeroFill(3); ig.system.context.globalAlpha = a; this.fontTitle.draw('WAVE ' + w + ' CLEAR', 32, ys, ig.Font.ALIGN.LEFT); ig.system.context.drawImage(this.separatorBar.data, 32, ys + 48, 276, 2); this.font.draw('SCORE: ' + this.score.zeroFill(6), 32, (ys * 1.2) + 10, ig.Font.ALIGN.LEFT); ig.system.context.globalAlpha = 1; }
            ...
            this.iap = this.iap || new Ejecta.IAPManager(); ig.game.waitingForItunes = true; this.iap.getProducts(['removeAds'], function (error, products) {
                if (error) { ig.game.waitingForItunes = false; ig.game.setTitle(); }
                else if (products.length) {
                    products[0].purchase(1, function (error, transaction) {
                        ig.game.waitingForItunes = false; if (error) { console.log(error); }
            ...
            this.iap = this.iap || new Ejecta.IAPManager(); ig.game.waitingForItunes = true; this.iap.restoreTransactions(function (error, transactions) {
                ig.game.waitingForItunes = false; if (error) { console.log(error); }
        ...
        }, openFeedbackForm: function (ev) { var close = document.createElement('div'); close.classList.add('ztype-modal-close'); close.addEventListener('click', this.closeFeedbackForm.bind(this)); close.innerHTML = '&#215;'; var iframe = document.createElement('iframe'); iframe.classList.add('ztype-modal-frame'); iframe.src = '/?feedback'; var modal = document.createElement('div'); modal.classList.add('ztype-modal'); modal.appendChild(close); modal.appendChild(iframe); var modalContainer = document.createElement('div'); modalContainer.classList.add('ztype-modal-container'); modalContainer.appendChild(modal); this.modalFeedbackContainer = modalContainer; document.body.appendChild(modalContainer); }, closeFeedbackForm: function (ev) {
            if (!this.modalFeedbackContainer) { return; }
            this.modalFeedbackContainer.remove(); this.modalFeedbackContainer = null;
        }
    }); ZType.MODE = { TITLE: 0, GAME: 1, GAME_OVER: 2 }; ZType.MULTIPLIER_TIERS = { 20: 2, 50: 3 }; ZType.FLAG = { CASE_SENSITIVE: 1 << 0, PUNCTUATION: 1 << 1, NUMBERS: 1 << 2 }; ZType.WAVES = { MOBILE: { fragment: 0, wave: 0, spawn: [], spawnWait: 1, healthBoost: 0, speedIncrease: 1.01, types: [{ type: EntityEnemyOppressor, count: 0, incEvery: 9 }, { type: EntityEnemyDestroyer, count: 0, incEvery: 4 }, { type: EntityEnemyMine, count: 3, incEvery: 1 }] }, DESKTOP: { fragment: 0, wave: 0, spawn: [], spawnWait: 0.7, healthBoost: 0, speedIncrease: 1.05, types: [{ type: EntityEnemyOppressor, count: 0, incEvery: 7 }, { type: EntityEnemyDestroyer, count: 0, incEvery: 3 }, { type: EntityEnemyMine, count: 3, incEvery: 1 }] } }; var canvas = ig.$('#ztype-game-canvas') || ig.$('#canvas'); var width = 480; var height = 720; if (ig.ua.mobile) {
        if (ig.$('#ztype-gsense') && ig.$('#ztype-byline')) { ig.$('#ztype-gsense').style.display = 'none'; ig.$('#ztype-byline').style.display = 'none'; }
